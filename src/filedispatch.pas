...
var
    i : uint32;
begin
    tracer.push_trace('filedispatch.registerHandler');
...
var
    ...
    pid       : uint32;
begin
    tracer.push_trace('filedispatch.dispatch');
    dispatch := 0;

    { 1. Check that the path is a valid file }
    validity := vfs.PathValid(absPath);
    ...
    bytesRead := vfs.ReadFile(fh, 0, @headerBuf[0], MAX_MAGIC_LEN);
    vfs.CloseFile(fh);
...
var i : uint32;
begin
    tracer.push_trace('filedispatch.init');
    syslog.logln('FILEDISPATCH', 'INIT BEGIN.');
    for i := 0 to MAX_HANDLERS - 1 do
        Handlers[i].Used := false;
    HandlerCount := 0;
    syslog.logln('FILEDISPATCH', 'INIT END.');
end;
