{
    ...
    All kernel/driver logging goes through this unit. Output is dispatched
    to all registered hooks (serial, file, debug panel, etc.) simultaneously.
...
{ Initialization — registers serial as default hook }
procedure init;
...
procedure logChar(c: char);
begin
    { Also send each char to serial directly for immediate byte-level output }
    serial.send(COM1, uint8(c), 10000);
    if c = #10 then begin
        dispatchLine;
...
begin
    { serial.sendString already sends CR+LF, but our dispatchLine sends
...
begin
    ...
    { Note: We don't register serialHook here because logChar already sends
      each character to serial directly. The hook system is for line-level
      consumers (file logging, debug panels, etc.) that want complete lines.
      If we registered serialHook, serial would get double output. }
end;
