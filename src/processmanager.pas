{
    ...
    kernel stacks, provides find/send/terminate/kill APIs, and implements
...
var
    ...
    prio     : uint8;
begin
    push_trace('processmanager.create');
    ...
    { Build fake interrupt frame on the stack so the first context switch
      ...
      Stack layout (growing downward):
        ...
        [StackTop - 40]  ESI     (0)
        [StackTop - 44]  EDI     (0)
        [StackTop - 48]  DS      ($10 = kernel data, padded to 32-bit)
        [StackTop - 52]  ES      ($10)
        [StackTop - 56]  FS      ($10)
        [StackTop - 60]  GS      ($10)
    ...
    syslog.log('PROCMGR', 'Created process: ');
...
var
    ...
    stStr   : pchar;
begin
    push_trace('processmanager.terminal_command_ps');
...
var
    ...
    pid    : uint32;
begin
    push_trace('processmanager.terminal_command_kill');
...
var
    ...
    pid    : uint32;
begin
    push_trace('processmanager.terminal_command_terminate');
...
var
    slotPtr : ^uint32;
begin
    push_trace('processmanager.init');
    syslog.logln('PROCMGR', 'INIT BEGIN.');

    { Create process table }
    Processes := DL_New(SizeOf(uint32));
    SchedulerIndex := 0;

    { Create idle process — PID 0, unkillable, uses kernel main stack }
    ...
    syslog.logln('PROCMGR', 'Idle process created (PID 0).');

    syslog.logln('PROCMGR', 'INIT END.');
    pop_trace;
end;
