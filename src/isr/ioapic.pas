{
    ...
    On systems where VirtualBox (or real hardware) routes PCI interrupts
    through the I/O APIC instead of the legacy 8259 PIC, we must program
    the IOAPIC redirection entry for each PCI IRQ we want to receive.

    ISA interrupts (timer, keyboard) continue to work through the 8259
...
begin
    ...
    { Build low dword of redirection entry:
        ...
        bit  13    — pin polarity   (1 = active-low for PCI)
        bit  15    — trigger mode   (1 = level for PCI)
