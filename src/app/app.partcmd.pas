...
begin
    ...
    for i := 0 to 3 do begin
        ...
        print('        ');
        if driver.storage.vol.mbr.get_bootable(@part) then
            println('yes')
        else
            println('no');
    end;
...
begin
    ...
    part.system_id := 0;
    driver.storage.vol.mgr.add_partition(device, slot, part);
...
begin
    ...
    old := driver.storage.vol.mgr.get_partition(device, slot);
    ...
    driver.storage.vol.mgr.remove_partition(device, slot);
...
begin
    ...
    part := driver.storage.vol.mgr.get_partition(device, slot);
    ...
    vol := nil;
    for i := 0 to driver.storage.vol.mgr.get_volume_count() - 1 do begin
    ...
    if vol = nil then begin
        driver.storage.vol.mgr.create_volume_from_partition(device, part.LBA_start, part.sector_count);
        vol := driver.storage.vol.mgr.get_volume(driver.storage.vol.mgr.get_volume_count() - 1);
    end;
    ...
    fs := driver.storage.fs.mgr.find_filesystem_by_name(fsName);
    if fs = nil then begin
        print('Error: filesystem "');
        print(fsName);
        println('" not registered.');
        print('Registered: ');
        if driver.storage.fs.mgr.get_filesystem_count() = 0 then
            println('(none)')
        else begin
            for i := 0 to driver.storage.fs.mgr.get_filesystem_count() - 1 do begin
                if i > 0 then print(', ');
                print(driver.storage.fs.mgr.get_filesystem(i)^.sName);
            end;
            println('');
        end;
        exit;
...
procedure init();
begin
    io.stdio.registerCommand('PART', @command_part, 'Partition management.');
end;
