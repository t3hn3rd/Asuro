{
    ...
    proc.mgr.runCommand.  A per-instance LVGL timer polls the
...
uses
    driver.video.lvgl, driver.video, driver.video.windows, driver.video.desktop, driver.hid.keyboard, debug.tracer,
    core.strings, core.util, arch.x86.util, memory.heap, core.version, io.stdio, driver.storage.vfs,
    proc.mgr, proc.types, core.ds.lists, core.ds.hashmap, driver.storage.filedispatch;
...
begin
    ...
    ctx := proc.mgr.findByID(state^.ForegroundPID);
    dirty := false;
    ...
    if finished then begin
        if state^.fg_stdout <> nil then begin
            io.stdio.freeOutBuf(state^.fg_stdout);
            state^.fg_stdout := nil;
        end;
        if state^.fg_stderr <> nil then begin
            io.stdio.freeOutBuf(state^.fg_stderr);
            state^.fg_stderr := nil;
        end;
        if state^.fg_stdin <> nil then begin
            io.stdio.freeOutBuf(state^.fg_stdin);
            state^.fg_stdin := nil;
        end;
        if state^.fg_params <> nil then begin
            io.stdio.freeParams(state^.fg_params);
            state^.fg_params := nil;
        end;
...
begin
    ...
    for i := sint32(DL_Size(state^.bg_jobs)) - 1 downto 0 do begin
        ...
        if finished then begin
            ...
            dirty := true;
            { Free I/O buffers }
            if job^.StdOut <> nil then io.stdio.freeOutBuf(job^.StdOut);
            if job^.StdErr <> nil then io.stdio.freeOutBuf(job^.StdErr);
            kfree(void(job));
...
begin
    ...
    ctx := proc.mgr.findByID(state^.ForegroundPID);
    if ctx <> nil then
        proc.mgr.kill(state^.ForegroundPID);
    { Drain any final output before freeing buffers }
    drainOutput(state);
    { If drainOutput didn’t already clean up (process may not be reaped yet) }
    if state^.fg_stdout <> nil then begin io.stdio.freeOutBuf(state^.fg_stdout); state^.fg_stdout := nil; end;
    if state^.fg_stderr <> nil then begin io.stdio.freeOutBuf(state^.fg_stderr); state^.fg_stderr := nil; end;
    if state^.fg_stdin  <> nil then begin io.stdio.freeOutBuf(state^.fg_stdin);  state^.fg_stdin  := nil; end;
    if state^.fg_params <> nil then begin io.stdio.freeParams(state^.fg_params); state^.fg_params := nil; end;
    state^.ForegroundPID := 0;
    state^.last_drain := 0;
    state^.last_drain_err := 0;
    appendChar(state, #10);
...
var
    ...
    i    : uint32;
begin
    Map := driver.storage.vfs.GetDirectoryListingFrom('.', state^.cwd);
...
begin
    ...
    { Parse parameters }
    params := io.stdio.getParams(state^.line_buf);
    ...
    if (params <> nil) and (params^.Param <> nil) then begin
        ...
        is_bg := false;
        pcount := io.stdio.paramCount(params);
        if pcount > 0 then begin
            lastParam := io.stdio.getParam(pcount - 1, params);
            if (lastParam <> nil) and (lastParam[0] = '&') and (lastParam[1] = #0) then
                is_bg := true;
        end;

        cmd := io.stdio.findCommand(uppera);
        kfree(void(uppera));
        ...
        if cmd <> nil then begin
            if is_bg then begin
                ...
                if job <> nil then begin
                    ...
                    job^.StdOut := io.stdio.createOutBuf(1024);
                    job^.StdErr := io.stdio.createOutBuf(1024);
                    job^.LastDrain := 0;
                    job^.LastDrainE := 0;
                    ...
                    if ctx <> nil then begin
                        ...
                        appendChar(state, #10);
                        io.stdio.freeOutBuf(job^.StdOut);
                        io.stdio.freeOutBuf(job^.StdErr);
                        kfree(void(job));
                    end;
                end;
                io.stdio.freeParams(params);
                ...
                { --- Foreground launch --- }
                state^.fg_stdout := io.stdio.createOutBuf(1024);
                state^.fg_stderr := io.stdio.createOutBuf(1024);
                state^.fg_stdin  := io.stdio.createOutBuf(0);
                state^.last_drain := 0;
                state^.last_drain_err := 0;
                ...
                if ctx <> nil then begin
                    ...
                    appendText(state, 'Error: failed to create process.');
                    appendChar(state, #10);
                    io.stdio.freeOutBuf(state^.fg_stdout);  state^.fg_stdout := nil;
                    io.stdio.freeOutBuf(state^.fg_stderr);  state^.fg_stderr := nil;
                    io.stdio.freeOutBuf(state^.fg_stdin);   state^.fg_stdin := nil;
                    io.stdio.freeParams(params);
                    state^.fg_params := nil;
                    state^.ForegroundPID := 0;
                    showPrompt(state);
            ...
            if absPath <> nil then begin
                if is_bg then begin
                    ...
                    if job <> nil then begin
                        ...
                        dispatchPID := driver.storage.filedispatch.dispatch(absPath, params,
                                                             nil, job^.StdOut, job^.StdErr);
        ...
        if params <> nil then io.stdio.freeParams(params);
        showPrompt(state);
    end;
...
var
    ...
    scr_w   : sint32;
begin
    cnt := driver.video.windows.getWindowContent(wid);
    state := nil;
    if cnt <> nil then
    ...
    if state <> nil then begin
        ...
        if state^.TerminalPID <> 0 then begin
            proc.mgr.kill(state^.TerminalPID);
            state^.TerminalPID := 0;
        end;
            ...
            for scr_w := sint32(DL_Size(state^.bg_jobs)) - 1 downto 0 do begin
                ...
                if slotPtr <> nil then begin
                    ...
                    if bgJob <> nil then begin
                        proc.mgr.kill(bgJob^.PID);
                        if bgJob^.StdOut <> nil then io.stdio.freeOutBuf(bgJob^.StdOut);
                        if bgJob^.StdErr <> nil then io.stdio.freeOutBuf(bgJob^.StdErr);
                        kfree(void(bgJob));
                    end;
                end;
            end;
...
begin
    ...
    state^.win_id := driver.video.windows.createWindow(
        'Terminal',
        wx, wy, TERM_W, TERM_H,
        @onClose,
        nil
    ...
    state^.content := driver.video.windows.getWindowContent(state^.win_id);
    if state^.content = nil then begin
        driver.video.windows.destroyWindow(state^.win_id);
        kfree(void(state));
        exit;
    end;
    ...
    ctx := proc.mgr.create('Terminal', @terminal_entry, void(state), 1);
    if ctx <> nil then begin
        state^.TerminalPID := ctx^.ProcessID;
        driver.video.windows.setWindowOwner(state^.win_id, ctx^.ProcessID);
    end else
        state^.TerminalPID := 0;
...
{ ============================================================
  init — register Terminal in the driver.video.desktop program launcher
  ============================================================ }
procedure init;
begin
    debug.tracer.push_trace('app.vterminal.init');
    driver.video.desktop.registerProgram('Terminal', @launch);
