{
    prog->Splash - Boot splash screen.

    Displays a centered logo (decoded from an embedded TGA), progress bar
    and status label during kernel initialization.  Because the gfxd
...
uses
    lvgl, video, targa, texture, lmemorymanager;

{ Create the splash screen and render the initial frame (0 %). }
procedure init;
...
{ Remove all splash LVGL objects and load the default screen. }
procedure teardown;
...
var
    { The dedicated LVGL screen that holds the splash widgets. }
...
{ Crude busy-wait delay.  Reads the PIT counter (channel 0) in a
  ...
  DEBUG ONLY — remove once splash timing is no longer needed. }
...
begin
    ...
    logoTexture := targa.Parse(@splash_tga_start, splash_tga_size);
    logoDsc := nil;

    { Create a new screen for the splash and make it current. }
...
begin
    ...
    { Delete the splash screen and all of its children. }
