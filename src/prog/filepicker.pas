...
const
    FP_FILTER_ALL = nil;   { pass as filter param to show all file types }
...
{ ============================================================
  ...
  kfree the stringCopy'd strings stored in child user_data.
  ============================================================ }
...
{ ============================================================
  fp_collect_cb
  hashmap.forEach callback: classifies each VFS entry as dir or file
...
var
    Map        : PHashMap;
    ctx        : TFPCollectCtx;
    dir_names  : PNameBuf;   { heap-alloc'd to avoid blowing the kernel stack }
    file_names : PNameBuf;
    dir_count  : uint32;
    file_count : uint32;
    i          : uint32;
...
begin
    ...
    syslog.logln('FPCIK', 'do_refresh: enter');
    if p^.file_list = nil then begin
        syslog.logln('FPCIK', 'do_refresh: file_list nil - exit');
        tracer.pop_trace; exit;
    end;

    syslog.logln('FPCIK', 'do_refresh: kalloc name bufs');
    ...
    syslog.logln('FPCIK', 'do_refresh: calling GetDirectoryListingFrom');
    syslog.logln('FPCIK', p^.cur_dir);
    Map := vfs.GetDirectoryListingFrom(p^.cur_dir, '/');
    syslog.logln('FPCIK', 'do_refresh: GetDirectoryListingFrom returned');
    if Map = nil then begin
        syslog.logln('FPCIK', 'do_refresh: map nil - bad dir');
    ...
    if Map^.Table = nil then begin
        syslog.logln('FPCIK', 'do_refresh: map table nil - corrupt map');
    ...
    syslog.logln('FPCIK', 'do_refresh: map ok, collecting entries');
    ...
    ctx.file_count := 0;
    ctx.filter     := p^.filter;
    hashmap.forEach(Map, @fp_collect_cb, void(@ctx));
    dir_count  := ctx.dir_count;
    file_count := ctx.file_count;

    syslog.logln('FPCIK', 'do_refresh: collection done, sorting');
    ...
    syslog.logln('FPCIK', 'do_refresh: building UI rows done, freeing map');
    vfs.FreeDirectoryListing(Map);
    ...
    syslog.logln('FPCIK', 'do_refresh: exit');
    tracer.pop_trace;
end;
...
var
    p: PPicker;
begin
    syslog.logln('FPCIK', 'fp_refresh_timer_cb: fired');
    p := PPicker(lv_timer_get_user_data(tmr));
    lv_timer_delete(tmr);
    if p = nil then begin
        syslog.logln('FPCIK', 'fp_refresh_timer_cb: p nil');
        exit;
    end;
...
begin
    ...
    if p^.is_save then begin
        ...
        if (not p^.confirm_pending) and
           (vfs.PathValid(path) = pvFile) then begin
...
begin
    ...
    err := vfs.newVirtualDirectory(fp);
    kfree(void(fp));
...
begin
    ...
    syslog.logln('FPCIK', 'make_picker: enter');
    ...
    syslog.logln('FPCIK', 'make_picker: scheduling initial refresh');
    schedule_refresh(p);

    syslog.logln('FPCIK', 'make_picker: exit');
    tracer.pop_trace;
end;
...
procedure show_open(title     : pchar;
                    ...
                    userdata  : pointer);
begin
    syslog.logln('FPCIK', 'show_open: enter');
...
procedure show_save(title        : pchar;
                    ...
                    userdata     : pointer);
begin
    syslog.logln('FPCIK', 'show_save: enter');
