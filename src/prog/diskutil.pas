...
begin
    controllerStr := storagemanager.controller_type_2_string(ct);
end;
...
begin
    ...
    vol := volumemanager.get_volume(sel_vol_idx);
...
begin
    ...
    freeSlot := volumemanager.find_free_slot(device);
    ...
    for i := 0 to 3 do begin
        if volumemanager.find_free_slot(device) >= 0 then
...
begin
    ...
    if filesystemmanager.get_filesystem_count() = 0 then begin
        mbox := lv_msgbox_create(lv_layer_top);
        active_mbox := mbox;
        lv_msgbox_add_title(mbox, 'Cannot Format');
...
begin
    ...
    { Boot indicator }
    if MBR.get_bootable(@part) then begin
        lbl := lv_label_create(row);
...
begin
    ...
    device := storagemanager.get_device(devIdx);
    if device = nil then begin
...
begin
    ...
    devIdx := 0;
    for i := 0 to storagemanager.get_device_count() - 1 do begin
        if storagemanager.get_device(i) = vol^.device then begin
            devIdx := i;
            break;
        end;
    end;
...
begin
    ...
    devCnt := storagemanager.get_device_count();
    for i := 0 to devCnt - 1 do begin
        ...
        for j := 0 to volCnt - 1 do begin
            vol := volumemanager.get_volume(j);
            if vol = nil then continue;
...
begin
    ...
    add_err_lbl := nil;
    active_sidebar_btn := nil;
    desktop.registerProgram('Disk Utility', @launch);
    tracer.pop_trace;
end;
