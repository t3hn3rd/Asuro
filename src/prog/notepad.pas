{
    ...
    Key design notes:
      ...
      - Shift state is read from keyboard.is_shift / keyboard.is_ctrl.
...
begin
    ...
    fHandle := vfs.OpenFile(state^.currentPath, omReadWrite, wmRewrite, @fError);
    if fHandle = 0 then
        fHandle := vfs.OpenFile(state^.currentPath, omWriteOnly, wmNew, @fError);
    if fHandle = 0 then begin
        kfree(void(buf));
        syslog.logln('NOTEPAD', 'saveFile: OpenFile failed');
        showErrorMsgbox('Save Failed', 'Could not open file for writing.');
        updateStatusBar(state);
        tracer.pop_trace;
        exit;
    end;
    written := vfs.WriteFile(fHandle, 0, puint8(buf), len);
    vfs.CloseFile(fHandle);
    kfree(void(buf));
    if written = 0 then begin
        syslog.logln('NOTEPAD', 'saveFile: WriteFile returned 0');
...
var
    ...
    buf     : pchar;
begin
    tracer.push_trace('notepad.loadFile');
    fHandle := vfs.OpenFile(path, omReadOnly, wmRewrite, @fError);
    if (fHandle = 0) or (fError <> eNone) then begin
        syslog.logln('NOTEPAD', 'loadFile: could not open file');
    ...
    buf := pchar(kalloc(FILE_BUF_SIZE));
    memset(uint32(buf), 0, FILE_BUF_SIZE);
    vfs.ReadFile(fHandle, 0, puint8(buf), FILE_BUF_SIZE - 1);
    vfs.CloseFile(fHandle);
    if state^.currentPath <> nil then kfree(void(state^.currentPath));
...
begin
    ...
    { ---- Ctrl+S: save ---- }
    if keyboard.is_ctrl and (key = ord('s')) then begin
        saveFile(state);
        stopEvent(e);
        exit;
    end;
    ...
    { ---- Ctrl+A: select all ---- }
    if keyboard.is_ctrl and (key = ord('a')) then begin
    ...
    if key = LV_KEY_NEXT then begin
        if state^.sel_anchor <> -1 then begin
            ...
            src      := lv_textarea_get_text(ta);
            new_text := applyIndent(src, sel_s, sel_e, keyboard.is_shift);
            lv_textarea_set_text(ta, new_text);
            kfree(void(new_text));
            state^.sel_anchor := -1;
            lv_textarea_clear_selection(ta);
...
{ ============================================================
  onClose — called by the windows unit when the X button is pressed
  ============================================================ }
procedure onClose(wid: uint32);
begin
    tracer.push_trace('notepad.onClose');
...
begin
    ...
    win_id := windows.createWindow('Notepad', wx, wy, WIN_W, WIN_H,
                                   @onClose, nil);
...
procedure init();
begin
    tracer.push_trace('notepad.init');
    win_id  := 0;
    g_state := nil;
    desktop.registerProgram('Notepad', @launch);
    tracer.pop_trace;
end;
