...

{
    ...
    Provides kalloc/kfree for core.version heap allocation.  Each heap page is a
...
{ Restore interrupt flag if it was enabled before cli.
  ...
  (IOPL, NT, VM, AC) that could cause arch.x86.fault.gpf or Bad-TSS on next IRET. }
...
var
    i: uint32;
begin
    push_trace('memory.heap.new_lmm_page');
    new_lmm_page := 0;
    i := KERNEL_PAGE_NUMBER + 4;
    while i < 1024 do begin
        if arch.x86.memory.virtual.new_page(i) then begin
    ...
    io.syslog.logln('LMM', 'ERROR: new_lmm_page failed.');
    pop_trace;
end;
...
var
    ...
    i: uint32;
begin
    push_trace('memory.heap.new_heap_page');
    addr := new_lmm_page;
...
begin
    ...
    push_trace('memory.heap.try_release_page');
    ...
    arch.x86.memory.virtual.free_page_at_address(uint32(hp));
...
procedure init;
begin
    push_trace('memory.heap.init');
    io.syslog.logln('LMM', 'INIT BEGIN.');
    Root_Page := new_heap_page(nil);
    Search_Page := Root_Page;
    if Root_Page = nil then begin
        io.syslog.logln('LMM', 'FATAL: Could not allocate root heap page!');
        GPF;
    end;
    io.syslog.logln('LMM', 'INIT END.');
    pop_trace;
end;
...
var
    block: uint16;
begin
    push_trace('memory.heap.kpalloc');
    block := address shr 22;
    if block >= 1024 then begin
        io.syslog.logln('LMM', 'ERROR: kpalloc block out of range.');
        GPF;
        kpalloc := nil;
        pop_trace;
        exit;
    end;
    io.syslog.log('LMM', 'kpalloc: identity-map block ');
    io.syslog.writeintln(block);
...
var
    ...
    i : uint32;
begin
    push_trace('memory.heap.klfree');
    num_pages := PuInt32(address)^;
    if (num_pages = 0) or (num_pages > 256) then begin
        io.syslog.logln('LMM', 'ERROR: klfree invalid page count.');
        GPF;
        pop_trace;
        exit;
    end;
    ...
    for i := 0 to num_pages - 1 do
        arch.x86.memory.virtual.free_page(page_num + uint16(i));
    pop_trace;
end;
...
var
    ...
    base_addr  : uint32;
begin
    push_trace('memory.heap.klalloc');
    klalloc := void(nil);
    ...
    { Scan low virtual pages (below core.version space) for contiguous free range }
    curr_page := 5;
    while uint32(curr_page) + pages - 1 < KERNEL_PAGE_NUMBER do begin
        ...
        if not miss then begin
            for i := 0 to pages - 1 do
                arch.x86.memory.virtual.new_page(curr_page + i);
    ...
    io.syslog.logln('LMM', 'ERROR: klalloc no contiguous pages.');
    pop_trace;
end;
...
begin
    ...
    if (prefix_val = 0) or (unit_idx + prefix_val > hp^.TotalUnits) then begin
        io.syslog.logln('LMM', 'ERROR: kfree invalid range.');
        GPF;
        restore_if(saved_flags);
        exit;
    end;
    ...
    if PuInt32(alloc_base + prefix_val * ALLOC_UNIT - 4)^ <> GUARD_MAGIC then begin
        io.syslog.logln('LMM', 'ERROR: kfree guard corrupted - buffer overrun!');
        GPF;
        restore_if(saved_flags);
        exit;
    end;
    ...
    if not bitmap_all_set(bmp, unit_idx, prefix_val) then begin
        io.syslog.logln('LMM', 'ERROR: kfree double free!');
        GPF;
        restore_if(saved_flags);
        exit;
    end;
