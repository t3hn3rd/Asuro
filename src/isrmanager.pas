...
type
    TISRNHookArray = Array[0..MAX_HOOKS] of pp_hook_method;
    TISRHookArray = Array[0..255] of TISRNHookArray;

procedure init;
procedure registerISR(INT_N : uint8; callback : pp_hook_method);
...
begin
    for i:=0 to MAX_HOOKS do begin
        if Hooks[INT_N][i] <> nil then Hooks[INT_N][i](nil);
    end;
    if (INT_N >= 8 + 32) then outb($A0, $20);
    outb($20, $20);
end;
