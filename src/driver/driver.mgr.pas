...
begin
    case Bus of
        biUnknown : io.stdio.bufWriteStr(outbuf, 'unknown');
        biANY     : io.stdio.bufWriteStr(outbuf, 'any');
        bii2c     : io.stdio.bufWriteStr(outbuf, 'i2c');
        biPCI     : io.stdio.bufWriteStr(outbuf, 'pci');
        biPCIe    : io.stdio.bufWriteStr(outbuf, 'pcie');
        biUSB     : io.stdio.bufWriteStr(outbuf, 'usb');
    end;
end;
...
begin
    ...
    while Drv <> nil do begin
        if Drv^.Loaded then begin
            ...
            io.stdio.bufWriteStr(stdout_buf, Drv^.Driver_Name);
            io.stdio.bufWriteStr(stdout_buf, ' - Bus: ');
            writeBusType(Drv^.Identifier^.Bus, stdout_buf);
            io.stdio.bufWriteStrLn(stdout_buf, ' ');
            io.stdio.bufWriteStr(stdout_buf, '   [');
            io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id0);
            io.stdio.bufWriteStr(stdout_buf, '-');
            io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id1);
            io.stdio.bufWriteStr(stdout_buf, '-');
            io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id2);
            io.stdio.bufWriteStr(stdout_buf, '-');
            io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id3);
            io.stdio.bufWriteStr(stdout_buf, '-');
            io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id4);
            ex:= Drv^.Identifier^.ex;
            while ex <> nil do begin
                io.stdio.bufWriteStr(stdout_buf, '-');
                io.stdio.bufWriteHex(stdout_buf, ex^.idN);
                ex:= ex^.ex;   
            end;
            io.stdio.bufWriteStrLn(stdout_buf, ']');
            i:= i + 1;
...
begin
    ...
    while Drv <> nil do begin
        ...
        io.stdio.bufWriteStr(stdout_buf, ' - Bus: ');
        writeBusType(Drv^.Identifier^.Bus, stdout_buf);
        io.stdio.bufWriteStr(stdout_buf, ' - Loaded: ');
        if Drv^.Loaded then io.stdio.bufWriteStrLn(stdout_buf, 'true') else io.stdio.bufWriteStrLn(stdout_buf, 'false');
        io.stdio.bufWriteStr(stdout_buf, '   [');
        io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id0);
        io.stdio.bufWriteStr(stdout_buf, '-');
        io.stdio.bufWriteHex(stdout_buf, Drv^.Identifier^.id1);
...
begin
    ...
    while Dv <> nil do begin
        io.stdio.bufWriteInt(stdout_buf, i);
        io.stdio.bufWriteStr(stdout_buf, ') ');
        io.stdio.bufWriteStr(stdout_buf, Dv^.Device_Name);
        io.stdio.bufWriteStr(stdout_buf, ' - Bus: ');
        writeBusType(Dv^.Identifier^.Bus, stdout_buf);
        io.stdio.bufWriteStrLn(stdout_buf, ' ');
        io.stdio.bufWriteStr(stdout_buf, '   [');
        io.stdio.bufWriteHex(stdout_buf, Dv^.Identifier^.id0);
        io.stdio.bufWriteStr(stdout_buf, '-');
        io.stdio.bufWriteHex(stdout_buf, Dv^.Identifier^.id1);
        io.stdio.bufWriteStr(stdout_buf, '-');
        io.stdio.bufWriteHex(stdout_buf, Dv^.Identifier^.id2);
        io.stdio.bufWriteStr(stdout_buf, '-');
        io.stdio.bufWriteHex(stdout_buf, Dv^.Identifier^.id3);
        io.stdio.bufWriteStr(stdout_buf, '-');
        io.stdio.bufWriteHex(stdout_buf, Dv^.Identifier^.id4);
        ex:= Dv^.Identifier^.ex;
        while ex <> nil do begin
            io.stdio.bufWriteStr(stdout_buf, '-');
            io.stdio.bufWriteHex(stdout_buf, ex^.idN);
        ...
        if Dv^.Driver_Loaded then begin
            io.stdio.bufWriteStr(stdout_buf, '   Driver Loaded: ');
            if Dv^.Driver <> nil then begin
                io.stdio.bufWriteStrLn(stdout_buf, Dv^.Driver^.Driver_Name);
            end else begin
                io.stdio.bufWriteStrLn(stdout_buf, 'Unknown')
            end;
        end;
        i:= i + 1;
        Dv:= Dv^.Next; 
...
begin
    if paramCount(Params) > 0 then begin
        ...
        io.stdio.bufWriteStrLn(stdout_buf, 'Driver Management Interface');
        io.stdio.bufWriteStrLn(stdout_buf, ' ');
        io.stdio.bufWriteStrLn(stdout_buf, 'An interface to the driver.mgr portion of the kernel.');
        io.stdio.bufWriteStrLn(stdout_buf, ' ');
        io.stdio.bufWriteStrLn(stdout_buf, 'Usage: ');
        io.stdio.bufWriteStrLn(stdout_buf, '      dev drivers  - Print a list of loaded drivers.');
        io.stdio.bufWriteStrLn(stdout_buf, '      dev devices  - Print a list of registered devices.');
        io.stdio.bufWriteStrLn(stdout_buf, '      dev driverex - Print a list of all available drivers.');
        io.stdio.bufWriteStrLn(stdout_buf, ' ')
    end;
end;
...
begin
    ...
    if DeviceID <> nil then begin;
        ...
        io.syslog.log('Driver Management', 'New Driver Registered: ');
        io.syslog.writestringln(NewReg^.Driver_Name);
        if force_load then begin
            io.syslog.log('Driver Management', 'Driver (');
            io.syslog.writestring(NewReg^.Driver_Name);
            io.syslog.writestringln(') forced to load.');
            NewReg^.Loaded:= True;
...
begin
    ...
    io.syslog.log('Driver Management', 'New Device Registered: ');
    io.syslog.writestringln(new_dev^.Device_Name);
    while drv <> nil do begin
        if identifiers_match(drv^.Identifier, DeviceID) then begin
            io.syslog.log('Driver Management', 'Device/Driver Match: ');
            io.syslog.writestring(new_dev^.Device_Name);
            io.syslog.writestring('->');
            io.syslog.writestringln(drv^.Driver_Name);
            if drv^.Driver_Load(ptr) then begin
                io.syslog.log('Driver Management', 'Driver (');
                io.syslog.writestring(drv^.Driver_Name);
                io.syslog.writestringln(') successfully loaded.');
                drv^.Loaded:= true;
