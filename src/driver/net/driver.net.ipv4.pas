    ...
    if paramCount(params) > 1 then begin
        ...
        if StringEquals(Command, 'set') then begin
            if paramCount(params) > 3 then begin
                ...
                io.stdio.bufWriteStrLn(stderr_buf, 'Invalid number of params to call ''set''.');
            end;
        end;
        if StringEquals(Command, 'driver.net') then begin
        ...
        io.stdio.bufWriteStr(stdout_buf, '   IPv4:    ');
        writeIPv4Address(@Config.Address[0], stdout_buf);
        io.stdio.bufWriteStr(stdout_buf, '   Gateway: ');
        writeIPv4Address(@Config.Gateway[0], stdout_buf);
        io.stdio.bufWriteStr(stdout_buf, '   Netmask: ');
        writeIPv4Address(@Config.Netmask[0], stdout_buf);
        if Config.UP then 
            io.stdio.bufWriteStrLn(stdout_buf, '   NetUP:   true') 
        else 
            io.stdio.bufWriteStrLn(stdout_buf, '   NetUP:   false');
    end;
end;
