...
var
    ...
    remaining    : uint16;
begin
    push_trace('tcp.CalculateChecksum');
...
var
    ...
    i    : uint32;
begin
    push_trace('tcp.AllocEphemeralPort');
...
var
    ...
    p   : void;
begin
    push_trace('tcp.CountPendingForPort');
...
var
    ...
    oldIdx: uint32;
begin
    push_trace('tcp.InsertOOOSegment');
    { Check for duplicate }
    for i := 0 to TCP_OOO_MAX - 1 do begin
...
var
    ...
    copyLen : uint16;
begin
    push_trace('tcp.FlushOOOSegments');
    flushed := false;
    repeat
        found := false;
        for i := 0 to TCP_OOO_MAX - 1 do begin
...
begin
    ...
    copyMAC(net.getMAC, @context^.MAC.Source[0]);
    ...
    ipv4.send(buffer, totalLen, context);
...
begin
    ...
    if sameSubnetIPv4(remoteIP, @getIPv4Config^.Address[0], @getIPv4Config^.Netmask[0]) then
        copyMAC(arp.resolveIP(remoteIP), @context^.MAC.Destination[0])
    else
        copyMAC(arp.resolveIP(@getIPv4Config^.Gateway[0]), @context^.MAC.Destination[0]);

    ipv4.send(buffer, sizeof(TTCPHeader), context);
    freePacketContext(context);
    kfree(buffer);
end;
...
{ ============================================================================ }
{  Timer Tick (called at 1024 Hz from TMR_0_ISR)                              }
{ ============================================================================ }
                ...
                if (tcb^.RetransTimer > 0) and (tcb^.RetransBuf <> nil) then begin
                    ...
                    if tcb^.RetransTimer = 0 then begin
                        if tcb^.RetransCount < TCP_MAX_RETRANS then begin
                            ...
                            copyMAC(net.getMAC, @context^.MAC.Source[0]);
                            copyMAC(@tcb^.RemoteMAC[0], @context^.MAC.Destination[0]);
                            ipv4.send(tcb^.RetransBuf, tcb^.RetransBufLen, context);
                            freePacketContext(context);
...
begin
    ...
    if sameSubnetIPv4(@context^.RemoteIP[0], @getIPv4Config^.Address[0], @getIPv4Config^.Netmask[0]) then
        copyMAC(arp.resolveIP(@context^.RemoteIP[0]), @tcb^.RemoteMAC[0])
    else
        copyMAC(arp.resolveIP(@getIPv4Config^.Gateway[0]), @tcb^.RemoteMAC[0]);
...
var
    tcb : PTCB;
begin
    push_trace('tcp.abort_connection');
...
begin
    buf := puint8(p_data);
    syslog.writestring('[TCP Recv] ');
    for i := 0 to p_len - 1 do
        syslog.logChar(char(buf[i]));
    syslog.writestringln('');
end;

procedure tcpconnect_on_event(socket : PTCPSocket; event : TTCPEvent);
begin
    push_trace('tcp.tcpconnect_on_event');
    case event of
        tteConnected: begin
            syslog.logln('TCP', 'Connected! Sending Hello, World!...');
            send(socket, void(@TCPTEST_HELLO[0]), 13);
        end;
        tteDisconnected:
            syslog.logln('TCP', 'Disconnected by remote host.');
        tteReset:
            syslog.logln('TCP', 'Connection reset by remote host.');
        tteTimeout:
            syslog.logln('TCP', 'Connection timed out.');
    end;
end;
...
begin
    push_trace('tcp.terminal_command_tcpconnect');
    if paramCount(params) < 2 then begin
        syslog.logln('TCP', 'Usage: tcpconnect <ip> <port>');
        exit;
    end;
    ...
    if ip = nil then begin
        syslog.logln('TCP', 'Invalid IP address.');
        exit;
    end;

    if port = 0 then begin
        syslog.logln('TCP', 'Invalid port number.');
        kfree(void(ip));
        exit;
    end;
    ...
    if dest_mac = nil then begin
        syslog.logln('TCP', 'Failed to resolve MAC address for target.');
        kfree(void(ip));
        exit;
    end;
    ...
    syslog.logln('TCP', 'Connecting...');

    TCPTEST_Socket := connect(@ctx);

    if TCPTEST_Socket = nil then
        syslog.logln('TCP', 'Failed to initiate connection.');
...
var
    ...
    i   : uint16;
begin
    push_trace('tcp.tcplisten_on_recv');
    buf := puint8(p_data);
    syslog.writestring('[TCP Listen Recv] ');
    for i := 0 to p_len - 1 do
        syslog.logChar(char(buf[i]));
    syslog.writestringln('');
end;
...
begin
    push_trace('tcp.tcplisten_on_event');
    case event of
        tteConnected:
            syslog.logln('TCP', 'Listen: client connected.');
        tteDisconnected:
            syslog.logln('TCP', 'Listen: client disconnected.');
        tteReset:
            syslog.logln('TCP', 'Listen: connection reset.');
        tteTimeout:
            syslog.logln('TCP', 'Listen: connection timed out.');
    end;
end;
...
begin
    push_trace('tcp.terminal_command_tcplisten');
    if paramCount(params) < 1 then begin
        syslog.logln('TCP', 'Usage: tcplisten <port>');
        exit;
    end;
    ...
    syslog.logln('TCP', 'Listening on port...');

    TCPLISTEN_Socket := listen(@ctx);

    if TCPLISTEN_Socket = nil then
        syslog.logln('TCP', 'Failed to start listener.');
end;
...
var
    ...
    i   : uint16;
begin
    push_trace('tcp.tcphttp_on_recv');
    buf := puint8(p_data);
    syslog.writestring('[HTTP Response] ');
    for i := 0 to p_len - 1 do
        syslog.logChar(char(buf[i]));
    syslog.writestringln('');
end;
...
var
    ...
    src     : pchar;
begin
    push_trace('tcp.tcphttp_on_event');
    case event of
        tteConnected: begin
            syslog.logln('TCP', 'HTTP: Connected, sending GET request...');
            { Build: GET / HTTP/1.0\r\nHost: <host>\r\nConnection: close\r\n\r\n }
            reqLen := 0;
        ...
        tteDisconnected:
            syslog.logln('TCP', 'HTTP: Connection closed by server.');
        tteReset:
            syslog.logln('TCP', 'HTTP: Connection reset.');
        tteTimeout:
            syslog.logln('TCP', 'HTTP: Connection timed out.');
    end;
end;
...
var
    ...
    i        : uint16;
begin
    push_trace('tcp.terminal_command_tcphttp');
    if paramCount(params) < 1 then begin
        syslog.logln('TCP', 'Usage: tcphttp <ip> [port]');
        exit;
    end;
    ...
    syslog.logln('TCP', 'HTTP: Connecting...');

    TCPHTTP_Socket := connect(@ctx);

    if TCPHTTP_Socket = nil then
        syslog.logln('TCP', 'HTTP: Failed to initiate connection.');
