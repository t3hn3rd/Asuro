...
unit driver.net.util;

interface

uses
    debug.tracer, core.util, arch.x86.util, driver.net.types, io.stdio, memory.heap, core.ds.lists, core.strings;
...
begin
    push_trace('driver.net.util.IPEqual');
    IPEqual:= true;
    for i:=0 to 3 do begin
...
begin
    push_trace('driver.net.util.MACEqual');
    MACEqual:= true;
    for i:=0 to 5 do begin
...
begin
    push_trace('driver.net.util.writeIPv4Address');
    io.stdio.bufWriteInt(outbuf, ip[0]);
    for i:=1 to 3 do begin
        io.stdio.bufWriteStr(outbuf, '.');
        io.stdio.bufWriteInt(outbuf, ip[i]);
    end; 
end;
...
begin
    push_trace('driver.net.util.writeMACAddress');
    io.stdio.bufWriteHexPair(outbuf, mac[0]);
    for i:=1 to 5 do begin
        io.stdio.bufWriteStr(outbuf, ':');
        io.stdio.bufWriteHexPair(outbuf, mac[i]);
    end;
end;
...
function newPacketContext : PPacketContext;
begin
    push_trace('driver.net.util.newPacketContext');
...
procedure freePacketContext(p_context : PPacketContext);
begin
    push_trace('driver.net.util.freePacketContext');
...
begin
    push_trace('driver.net.util.copyMAC');
    for i:=0 to 5 do begin
        dst[i]:= src[i];
    end;
    pop_trace;
...
begin
    push_trace('driver.net.util.copyIPv4');
    for i:=0 to 3 do begin
        dst[i]:= src[i];
    end;
    pop_trace;
