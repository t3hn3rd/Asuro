{
    Driver->HID->MouseState - Decoupled mouse state & event hooks.

    Owns the global mouse position, button state, and scroll accumulator.
    Consumers (LVGL, future UIs) register hooks via registerMouseHook.
    Writers (mouse.pas ISR) call the setters and fireMouseEvent.
...
{ Setters — called from the mouse ISR }
