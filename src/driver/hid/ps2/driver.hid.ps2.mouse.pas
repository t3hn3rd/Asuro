{
    ...
    Handles PS/2 driver.hid.mouse packets and delivers events through
    the abstract driver.hid.mouse API (driver.hid.mouse.setMousePos, driver.hid.mouse.fireMouseEvent, etc.).

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.hid.ps2.mouse;
    ...
    while mouse_wait(0) do begin
        ...
        if Cycle = PacketSize then begin
            ...
            if not(Packet.x_overflow) and not(Packet.y_overflow) then begin
                ...
                if Current.x > sint32(driver.video.frontBufferWidth - 1) then Current.x := sint32(driver.video.frontBufferWidth - 1);
                if Current.y > sint32(driver.video.frontBufferHeight - 1) then Current.y := sint32(driver.video.frontBufferHeight - 1);
            end;
            { Process scroll wheel (4th byte, signed) }
            if HasScrollWheel then begin
                driver.hid.mouse.addScroll(sint8(Mouse_Byte[3]));
            end;
            Cycle := 0;
            ...
            driver.hid.mouse.setMousePos(Current.x, Current.y);
...
var
    ...
    tmp : uint8;
begin
    push_trace('driver.hid.ps2.mouse.load');

    { Disable both PS/2 ports while configuring }
    mouse_wait_long(1);
    outb($64, $AD);  { Disable driver.hid.keyboard port }
    mouse_wait_long(1);
    outb($64, $A7);  { Disable driver.hid.mouse port }
    ...
    { Enable IRQ12 (bit 1), clear disable-driver.hid.mouse-clock (bit 5) }
    status := status OR $02;
    ...
    { Enable auxiliary (driver.hid.mouse) port }
    mouse_wait_long(1);
    outb($64, $A8);

    { Re-enable driver.hid.keyboard port }
    mouse_wait_long(1);
    outb($64, $AE);

    { Reset driver.hid.mouse and wait for self-test }
    mouse_write($FF);
    ...
    if devid_byte = 3 then begin
        HasScrollWheel := true;
        PacketSize := 4;
        io.syslog.logln('PS/2 MOUSE', 'Scroll wheel enabled (IntelliMouse).');
    end else begin
        HasScrollWheel := false;
        PacketSize := 3;
        io.syslog.logln('PS/2 MOUSE', 'Standard driver.hid.mouse (no scroll wheel).');
    end;

    mouse_write($F4);
    mouse_read();
    arch.x86.isr.mgr.registerISR(44, @Main);
    io.syslog.logln('PS/2 MOUSE', 'LOADED.');
    io.syslog.log('PS/2 MOUSE', 'Memory: ');
    io.syslog.writehexln(uint32(@current));
    load := true;
    pop_trace;
end;
...
begin
    push_trace('driver.hid.ps2.mouse.init');
    io.syslog.logln('PS/2 MOUSE', 'INIT BEGIN.');
    devid.bus := biUnknown;
    devid.id0 := 0;
    devid.id1 := 0;
    devid.id2 := 0;
    ...
    Last.x := 0;
    Last.y := 0;
    driver.mgr.register_driver_ex('PS/2 Mouse', @devid, @load, true);
