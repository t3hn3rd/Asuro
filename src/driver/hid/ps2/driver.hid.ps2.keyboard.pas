{
    ...
    through the abstract driver.hid.keyboard API (driver.hid.keyboard.reportKeyEvent).

    @author(Aaron Hance <ah@aaronhance.me>)
    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.hid.ps2.keyboard;
...
begin
    ...
    if raw = 42  then driver.hid.keyboard.is_shift := true;
    if raw = 170 then driver.hid.keyboard.is_shift := false;
    if raw = 29  then driver.hid.keyboard.is_ctrl  := true;
    if raw = 157 then driver.hid.keyboard.is_ctrl  := false;
    if raw = 56  then driver.hid.keyboard.is_alt   := true;
    if raw = 184 then driver.hid.keyboard.is_alt   := false;

    info.SHIFT_DOWN    := driver.hid.keyboard.is_shift;
    info.CTRL_DOWN     := driver.hid.keyboard.is_ctrl;
    info.ALT_DOWN      := driver.hid.keyboard.is_alt;
    info.is_down_code  := not isBreak;

    if driver.hid.keyboard.is_shift then begin
...
begin
    arch.x86.isr.ps2keyboard.register();
    arch.x86.isr.ps2keyboard.hook(uint32(@callback));
    io.syslog.logln('PS/2 KEYBOARD', 'LOADED.');
    load := true;
end;

procedure disable;
begin
    arch.x86.isr.ps2keyboard.unhook(uint32(@callback));
...
begin
    io.syslog.logln('PS/2 KEYBOARD', 'INIT BEGIN.');
