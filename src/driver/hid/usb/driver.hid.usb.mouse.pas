{
    Driver->Bus->driver.bus.usb->USB_Mouse - driver.bus.usb HID Boot Mouse Class Driver.

    Registers with driver.mgr as a driver.bus.usb class driver matching
    HID boot driver.hid.mouse devices (class $03, subclass $01, protocol $02).
    Uses boot protocol with 3/4-byte reports.
    Integrates with driver.hid.mouse API for position/button/scroll state.

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.hid.usb.mouse;
...
const
    ...
    { Boot driver.hid.mouse report: at least 3 bytes, may have 4th for wheel }
    MOUSE_REPORT_SIZE     = 4;
...
{ Extract signed X delta from boot driver.hid.mouse report byte 1. }
...
{ Extract signed Y delta from boot driver.hid.mouse report byte 2. }
...
{ Extract signed scroll delta from boot driver.hid.mouse report byte 3 (if present). }
...
{ Extract button state from boot driver.hid.mouse report byte 0. }
...
begin
    ...
    ms^.PosX := ms^.PosX + dx;
    ms^.PosY := ms^.PosY + dy;
    driver.hid.mouse.setMousePos(ms^.PosX, ms^.PosY);
    { Re-read clamped position back }
    ms^.PosX := driver.hid.mouse.getMouseX;
    ms^.PosY := driver.hid.mouse.getMouseY;
    ...
    if lmbDown then begin
        driver.hid.mouse.setMouseLMB(true);
        driver.hid.mouse.fireMouseEvent(MOUSE_DOWN_LEFT);
    end;
    if lmbUp then begin
        driver.hid.mouse.setMouseLMB(false);
        driver.hid.mouse.fireMouseEvent(MOUSE_CLICK_LEFT);
        driver.hid.mouse.fireMouseEvent(MOUSE_UP_LEFT);
    end;
    if rmbDown then begin
        driver.hid.mouse.setMouseRMB(true);
        driver.hid.mouse.fireMouseEvent(MOUSE_DOWN_RIGHT);
    end;
    if rmbUp then begin
        driver.hid.mouse.setMouseRMB(false);
        driver.hid.mouse.fireMouseEvent(MOUSE_CLICK_RIGHT);
        driver.hid.mouse.fireMouseEvent(MOUSE_UP_RIGHT);
    end;

    { Scroll wheel }
    if scroll <> 0 then begin
        driver.hid.mouse.addScroll(scroll);
    end;

    { Fire move event if there was movement }
    if (dx <> 0) or (dy <> 0) then begin
        driver.hid.mouse.fireMouseEvent(MOUSE_MOVE);
    end;
...
begin
    ...
    while i < cnt do begin
        ...
        if (ms <> nil) and (ms^.Device = dev) then begin
            io.syslog.logln('USBMouse', 'Device disconnected, deactivating.');
        ...
        if ms^.Transfer <> nil then begin
            if ms^.Transfer^.Status = tsSuccess then begin
                ...
                if ms^.FailCount >= 3 then begin
                    io.syslog.logln('USBMouse', 'Too many failures, deactivating.');
                    ms^.Transfer := nil;
                    ms^.Active := false;
                    ms^.Device := nil;
                    LL_Delete(MouseList, i);
...
var
    ...
    i      : uint32;
begin
    push_trace('driver.hid.usb.mouse.load');
    load := false;
    ...
    io.syslog.logln('USBMouse', 'Configuring driver.bus.usb mouse...');
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('USBMouse', 'SET_PROTOCOL(Boot) failed, continuing anyway.');
    end;
    ...
    status := usb_control_msg(dev,
        ...
        nil, 0);
    if status <> tsSuccess then begin
        io.syslog.logln('USBMouse', 'SET_IDLE failed, continuing anyway.');
    end;
    ...
    if not ms^.HasIntrEP then begin
        io.syslog.logln('USBMouse', 'No interrupt IN endpoint found.');
        pop_trace;
        exit;
    end;
    ...
    io.syslog.log('USBMouse', 'Mouse ready (EP');
    io.syslog.writeint(ms^.IntrEP.Address);
    io.syslog.writestring(', MaxPkt=');
    io.syslog.writeint(ms^.IntrEP.MaxPacket);
...
begin
    push_trace('driver.hid.usb.mouse.init');
    io.syslog.logln('USBMouse', 'INIT BEGIN.');

    MouseList := LL_New(sizeof(TUSBMouseData));

    { Register as a driver.bus.usb class driver matching HID boot driver.hid.mouse }
    ...
    driver.mgr.register_driver('driver.bus.usb Mouse Driver', @MouseID, @load);

    { Register completion hook so HC ISRs trigger driver.hid.mouse polling }
    driver.bus.usb.core.register_completion_hook(driver.bus.usb.core.TUSBCompletionHook(@poll_mice));

    io.syslog.logln('USBMouse', 'INIT END.');
    pop_trace;
end;
...
begin
    passed := 0;
    failed := 0;
    io.syslog.logln('USBMouse', 'Unit tests starting...');
