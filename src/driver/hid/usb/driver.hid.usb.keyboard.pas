{
    Driver->Bus->driver.bus.usb->USB_Keyboard - driver.bus.usb HID Boot Keyboard Class Driver.

    Registers with driver.mgr as a driver.bus.usb class driver matching
    HID boot driver.hid.keyboard devices (class $03, subclass $01, protocol $01).
    Uses boot protocol with 8-byte reports.

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.hid.usb.keyboard;
...
const
    ...
    { Boot driver.hid.keyboard report size }
    KB_REPORT_SIZE        = 8;
...
{ driver.bus.usb HID Usage Page 7 (Keyboard) to ASCII lookup table.
...
{ Process a completed boot driver.hid.keyboard report. }
    ...
    for i := 2 to 7 do begin
        ...
        if (usage <> 0) and (not key_was_pressed(kb, usage)) then begin
            ...
            if ascii <> 0 then begin
                ...
                info.CTRL_DOWN    := ctrl;
                info.ALT_DOWN     := alt;
                driver.hid.keyboard.reportKeyEvent(info);
            end;
        end;
    end;
...
begin
    ...
    while i < cnt do begin
        ...
        if (kb <> nil) and (kb^.Device = dev) then begin
            io.syslog.logln('USBKeyboard', 'Device disconnected, deactivating.');
        ...
        if kb^.Transfer <> nil then begin
            if kb^.Transfer^.Status = tsSuccess then begin
                ...
                if kb^.FailCount >= 3 then begin
                    ...
                    io.syslog.logln('USBKeyboard', 'Too many failures, deactivating.');
                    kb^.Transfer := nil;
                    kb^.Active := false;
                    kb^.Device := nil;
                    LL_Delete(KeyboardList, i);
...
var
    ...
    i       : uint32;
begin
    push_trace('driver.hid.usb.keyboard.load');
    load := false;
    ...
    io.syslog.logln('USBKeyboard', 'Configuring driver.bus.usb keyboard...');
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('USBKeyboard', 'SET_PROTOCOL(Boot) failed, continuing anyway.');
    end;
    ...
    status := usb_control_msg(dev,
        ...
        nil, 0);
    if status <> tsSuccess then begin
        io.syslog.logln('USBKeyboard', 'SET_IDLE failed, continuing anyway.');
    end;
    ...
    if not kb^.HasIntrEP then begin
        io.syslog.logln('USBKeyboard', 'No interrupt IN endpoint found.');
        pop_trace;
        exit;
    end;
    ...
    io.syslog.log('USBKeyboard', 'Keyboard ready (EP');
    io.syslog.writeint(kb^.IntrEP.Address);
    io.syslog.writestring(', MaxPkt=');
    io.syslog.writeint(kb^.IntrEP.MaxPacket);
    io.syslog.writestringln(').');

    { Disable PS/2 driver.hid.keyboard to prevent duplicate input from driver.bus.usb legacy emulation }
    driver.hid.ps2.keyboard.disable;
...
begin
    push_trace('driver.hid.usb.keyboard.init');
    io.syslog.logln('USBKeyboard', 'INIT BEGIN.');

    KeyboardList := LL_New(sizeof(TUSBKeyboardData));

    { Register as a driver.bus.usb class driver matching HID boot driver.hid.keyboard }
    ...
    driver.mgr.register_driver('driver.bus.usb Keyboard Driver', @KBID, @load);

    { Register completion hook so HC ISRs trigger driver.hid.keyboard polling }
    driver.bus.usb.core.register_completion_hook(driver.bus.usb.core.TUSBCompletionHook(@poll_keyboards));

    io.syslog.logln('USBKeyboard', 'INIT END.');
    pop_trace;
end;
...
var
    ...
    begin
        if condition then begin
            ...
            io.syslog.logln('USBKeyboard', msg);
            kfree(void(msg));
        end;
    end;
...
begin
    passed := 0;
    failed := 0;
    io.syslog.logln('USBKeyboard', 'Unit tests starting...');
