{
    ...
    Owns the global driver.hid.mouse position, button state, and scroll accumulator.
    All driver.hid.mouse drivers (PS/2, driver.bus.usb) push events through the setters and
    fireMouseEvent(). Consumers (LVGL, driver.video.windows, app.uidebug) use the getters
    and register hooks via registerMouseHook().

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.hid.mouse;
...
{ Setters - called by driver.hid.mouse drivers (PS/2, driver.bus.usb) }
...
var
    maxW, maxH : sint32;
begin
    maxW := sint32(driver.video.frontBufferWidth);
    maxH := sint32(driver.video.frontBufferHeight);
    if x < 0 then x := 0;
    if y < 0 then y := 0;
