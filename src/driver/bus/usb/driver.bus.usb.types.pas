{
    Driver->Bus->driver.bus.usb->USBTypes - Shared driver.bus.usb Types, Constants & HC Abstraction.

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.bus.usb.types;
...
type
    ...
    { ---- driver.bus.usb Device ---- }

    TUSBDevice = record
        Address      : uint8;       { Assigned driver.bus.usb address (1-127) }
    ...
    TUSBHCDriver = record
        Name         : PChar;
        HCType       : uint8;       { USB_HC_UHCI / driver.bus.usb.ohci / driver.bus.usb.ehci / driver.bus.usb.xhci }
        NumPorts     : uint8;
        PCIDev       : TPCI_Device; { Owning driver.bus.pci device }
        BaseAddr     : uint32;      { I/O base (driver.bus.usb.uhci) or MMIO base (driver.bus.usb.ohci/driver.bus.usb.ehci/driver.bus.usb.xhci) }
        PrivData     : Pointer;     { HC-specific state }
        Devices      : PLinkedListBase; { List of PUSBDevice attached }
        NextAddress  : uint8;       { Next available driver.bus.usb address (1-127) }
...
{ Build a driver.bus.usb setup packet. Returns a filled record. }
...
var
    ...
    overhead : uint32;
begin
    push_trace('driver.bus.usb.types.kalloc_aligned');
...
var
    raw : uint32;
begin
    push_trace('driver.bus.usb.types.kfree_aligned');
    if p <> nil then begin
...
procedure usb_hc_init_record(hc : PUSBHCDriver);
begin
    push_trace('driver.bus.usb.types.usb_hc_init_record');
...
begin
    passed := 0;
    failed := 0;
    io.syslog.logln('USBTYPES', 'Unit tests starting...');
