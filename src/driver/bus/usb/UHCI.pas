{
    Driver->Bus->USB->UHCI - Universal Host Controller Interface Driver.

    Implements the UHCI (USB 1.x) host controller for I/O-port-based controllers.
    UHCI uses a 1024-entry frame list, Queue Heads (QH) and Transfer Descriptors (TD).
...
const
    UHCI_REG_USBCMD     = $00;  { USB Command (16-bit) }
    UHCI_REG_USBSTS     = $02;  { USB Status (16-bit) }
    UHCI_REG_USBINTR    = $04;  { USB Interrupt Enable (16-bit) }
    ...
    { USBSTS bits }
    UHCI_STS_USBINT      = $0001; { USB Interrupt }
    UHCI_STS_ERROR        = $0002; { USB Error Interrupt }
...
type
    ...
    { Private data for a UHCI host controller instance }
...
{ Build a TD token dword.
  ...
  devAddr: USB device address (0-127)
...
begin
    ...
    if (uhci_read16(priv^.IOBase, UHCI_REG_USBCMD) AND UHCI_CMD_HCRESET) <> 0 then begin
        syslog.logln('UHCI', 'HC reset timeout!');
        pop_trace;
        exit;
    end;
...
begin
    ...
    if priv^.FrameList = nil then begin
        syslog.logln('UHCI', 'Failed to allocate frame list!');
        pop_trace;
        exit;
    end;
    ...
    if (priv^.QHInterrupt = nil) or (priv^.QHControl = nil) or (priv^.QHBulk = nil) then begin
        syslog.logln('UHCI', 'Failed to allocate QHs!');
        pop_trace;
        exit;
    end;
...
begin
    ...
    if (uhci_read16(priv^.IOBase, UHCI_REG_USBSTS) AND UHCI_STS_HCH) <> 0 then begin
        syslog.logln('UHCI', 'Controller failed to start (HCH still set).');
        pop_trace;
        exit;
    end;

    syslog.logln('UHCI', 'Controller started.');
    uhci_start := true;
    pop_trace;
end;
...
begin
    ...
    if (hc <> nil) and (hc^.PrivData <> nil) then begin
        priv := PUHCI_PrivData(hc^.PrivData);
        uhci_write16(priv^.IOBase, UHCI_REG_USBCMD, 0);
        syslog.logln('UHCI', 'Controller stopped.');
    end;
    pop_trace;
end;
...
begin
    ...
    case transfer^.PipeType of
        ...
        syslog.logln('UHCI', 'Unsupported pipe type for submit.');
    end;
    pop_trace;
end;
...
begin
    ...
    if (status AND UHCI_STS_HSE) <> 0 then begin
        syslog.logln('UHCI', 'Host System Error detected!');
...
begin
    ...
    syslog.writestringln(' UHCI controller(s).');
    ...
    for i := 0 to count - 1 do begin
        ...
        if iobase = 0 then begin
            syslog.logln('UHCI', 'Invalid I/O base (BAR4=0), skipping.');
            continue;
        end;
        ...
        if priv = nil then begin
            syslog.logln('UHCI', 'Failed to allocate private data!');
            continue;
        end;
        ...
        if not uhci_reset(@hc) then begin
            syslog.logln('UHCI', 'Reset failed, skipping controller.');
            kfree(void(priv));
            continue;
        end;
        ...
        if not uhci_start(@hc) then begin
            syslog.logln('UHCI', 'Start failed, skipping controller.');
...
begin
    passed := 0;
    failed := 0;
    syslog.logln('UHCI', 'Unit tests starting...');
