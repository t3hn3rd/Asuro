{
    Driver->Bus->USB->XHCI - eXtensible Host Controller Interface Driver.

    Implements the xHCI (USB 3.0) host controller using ring-based
    command/transfer/event model with TRBs (Transfer Request Blocks).

    xHCI manages device addresses internally via Enable Slot / Address Device
    commands. This driver intercepts SET_ADDRESS from usbcore and uses
...
{ ---- Operational Register Offsets (from OpBase = BAR0 + CAPLENGTH) ---- }
    ...
    XHCI_OP_USBCMD        = $00;  { USB Command }
    XHCI_OP_USBSTS        = $04;  { USB Status }
...
{ ---- HCCPARAMS1 Fields ---- }
    ...
    XHCI_HCC1_XECP_MASK     = $FFFF0000; { xHCI Extended Capabilities Pointer (bits 31:16) }
...
{ ---- Ring / Queue Sizes ---- }
    ...
    XHCI_MAX_SLOTS             = 256;  { xHCI spec max }
...
type
    ...
    { Private data for an xHCI host controller instance }
...
{ Map xHCI PORTSC speed field to USB_SPEED_* }
...
{ Map USB_SPEED_* to xHCI slot context speed value }
...
begin
    ...
    syslog.logln('XHCI', 'Command timeout!');
end;
...
var
    ...
    loops   : uint32;
begin
    push_trace('XHCI.xhci_bios_handoff');
    ...
    while loops < 100 do begin
        ...
        if (capVal AND $FF) = XHCI_LEGSUP_CAP_ID then begin
            { Found USB Legacy Support }
            if (capVal AND XHCI_LEGSUP_BIOS_OWNED) = 0 then begin
                syslog.logln('XHCI', 'BIOS does not own controller.');
                pop_trace;
                exit;
            end;
            ...
            while loops < 200000 do begin
                capVal := xhci_readl(capAddr, 0);
                if (capVal AND XHCI_LEGSUP_BIOS_OWNED) = 0 then begin
                    syslog.logln('XHCI', 'BIOS handoff complete.');
            ...
            { Force ownership }
            syslog.logln('XHCI', 'BIOS handoff timeout, forcing.');
            capVal := xhci_readl(capAddr, 0);
    ...
    syslog.logln('XHCI', 'No USBLEGSUP capability found.');
    pop_trace;
end;
...
var
    ...
    pagePhys : uint32;
begin
    push_trace('XHCI.xhci_alloc_scratchpad');
    ...
    if nScratch = 0 then begin
        syslog.logln('XHCI', 'No scratchpad buffers needed.');
        pop_trace;
        exit;
    end;

    syslog.log('XHCI', 'Allocating ');
    syslog.writeint(nScratch);
    syslog.writestringln(' scratchpad buffers.');
    ...
    if bufArray = nil then begin
        syslog.logln('XHCI', 'Failed to allocate scratchpad array!');
        pop_trace;
        exit;
    end;
    ...
    for i := 0 to nScratch - 1 do begin
        ...
        if page = nil then begin
            syslog.logln('XHCI', 'Failed to allocate scratchpad page!');
            pop_trace;
            exit;
        end;
    ...
    syslog.logln('XHCI', 'Scratchpad buffers allocated.');
    pop_trace;
end;
...
begin
    ...
    if (xhci_readl(opbase, XHCI_OP_USBSTS) AND XHCI_STS_HCH) = 0 then begin
        syslog.logln('XHCI', 'Controller did not halt!');
        pop_trace;
        exit;
    end;
    ...
    if ((xhci_readl(opbase, XHCI_OP_USBCMD) AND XHCI_CMD_HCRST) <> 0) or
       ((xhci_readl(opbase, XHCI_OP_USBSTS) AND XHCI_STS_CNR) <> 0) then begin
        syslog.logln('XHCI', 'HC reset timeout!');
        pop_trace;
        exit;
    end;
    ...
    if priv^.DCBAA = nil then begin
        syslog.logln('XHCI', 'Failed to allocate DCBAA!');
        pop_trace;
        exit;
    end;
    ...
    if priv^.CmdRing.Base = nil then begin
        syslog.logln('XHCI', 'Failed to allocate Command Ring!');
        pop_trace;
        exit;
    end;
    ...
    if priv^.EvtRing.Base = nil then begin
        syslog.logln('XHCI', 'Failed to allocate Event Ring!');
        pop_trace;
        exit;
    end;
    ...
    if priv^.ERST = nil then begin
        syslog.logln('XHCI', 'Failed to allocate ERST!');
        pop_trace;
        exit;
    end;
    ...
    syslog.logln('XHCI', 'HC reset complete.');
    xhci_reset := true;
    pop_trace;
end;
...
begin
    ...
    syslog.logln('XHCI', '  Interrupter 0 programmed (polling).');

    { Step 5: Start controller (RS only — no INTE since we poll) }
    cmd := XHCI_CMD_RS OR XHCI_CMD_HSEE;
    xhci_writel(opbase, XHCI_OP_USBCMD, cmd);
    syslog.logln('XHCI', '  USBCMD written, waiting for HCH clear...');
    ...
    if (xhci_readl(opbase, XHCI_OP_USBSTS) AND XHCI_STS_HCH) <> 0 then begin
        syslog.logln('XHCI', 'Controller failed to start!');
        pop_trace;
        exit;
    end;

    syslog.logln('XHCI', 'Controller started (operational).');
    xhci_start := true;
    pop_trace;
end;
...
begin
    ...
    if (hc <> nil) and (hc^.PrivData <> nil) then begin
        ...
        cmd := cmd AND (NOT XHCI_CMD_RS);
        xhci_writel(opbase, XHCI_OP_USBCMD, cmd);
        syslog.logln('XHCI', 'Controller stopped.');
    end;
    pop_trace;
end;
...
var
    priv : PXHCI_PrivData;
begin
    push_trace('XHCI.xhci_port_status');
    xhci_port_status := 0;
    if (hc = nil) or (hc^.PrivData = nil) then begin
        pop_trace;
        exit;
...
var
    ...
    cc       : uint8;
begin
    push_trace('XHCI.xhci_port_reset');
    ...
    if (portsc AND XHCI_PORTSC_PED) = 0 then begin
        syslog.log('XHCI', 'Port ');
        syslog.writeint(port);
        syslog.writestringln(' not enabled after reset.');
        pop_trace;
        exit;
    end;
    ...
    syslog.log('XHCI', 'Port ');
    syslog.writeint(port);
    syslog.writestring(' reset OK, speed=');
    syslog.writeintln(speed);
    ...
    if cc <> XHCI_CC_SUCCESS then begin
        syslog.log('XHCI', 'Enable Slot failed, cc=');
        syslog.writeintln(cc);
        pop_trace;
        exit;
    end;
    ...
    if (slotID = 0) or (slotID > priv^.MaxSlots) then begin
        syslog.logln('XHCI', 'Invalid slot ID from Enable Slot.');
        pop_trace;
        exit;
    end;

    syslog.log('XHCI', 'Slot ');
    syslog.writeint(slotID);
    syslog.writestringln(' enabled.');

    { ---- Allocate EP0 Transfer Ring ---- }
    ep0Ring := PXHCI_Ring(kalloc(sizeof(TXHCI_Ring)));
    if ep0Ring = nil then begin
        syslog.logln('XHCI', 'Failed to allocate EP0 ring descriptor!');
        pop_trace;
        exit;
    end;
    ep0Ring^ := xhci_alloc_ring(XHCI_TRANSFER_RING_SIZE);
    if ep0Ring^.Base = nil then begin
        syslog.logln('XHCI', 'Failed to allocate EP0 ring buffer!');
        kfree(void(ep0Ring));
        pop_trace;
        exit;
    end;
    ...
    if outCtx = nil then begin
        syslog.logln('XHCI', 'Failed to allocate output device context!');
    ...
    if inputCtx = nil then begin
        syslog.logln('XHCI', 'Failed to allocate input context!');
        xhci_free_ring(ep0Ring^);
        kfree(void(ep0Ring));
    ...
    if cc <> XHCI_CC_SUCCESS then begin
        syslog.log('XHCI', 'Address Device failed, cc=');
        syslog.writeintln(cc);
        { Clean up }
    ...
    syslog.log('XHCI', 'Device addressed at slot ');
    syslog.writeintln(slotID);
    ...
    { Set NextAddress so usbcore assigns slotID as the device "address" }
...
{ Map endpoint pipe type + direction to xHCI Endpoint Context Type value }
...
var
    ...
    speed      : uint8;
begin
    push_trace('XHCI.xhci_configure_endpoint');
    ...
    inputCtx := xhci_alloc_ctx(uint32(priv^.CtxSize) * 33);
    if inputCtx = nil then begin
        syslog.logln('XHCI', 'Failed to alloc input ctx for Configure EP!');
        xhci_free_ring(ring^);
        kfree(void(ring));
        pop_trace;
        exit;
    ...
    { Compute interval: for interrupt endpoints, xHCI uses 2^(interval-1) * 125us frames.
      USB boot keyboard typically has bInterval=10 (ms). For HS, interval field = bInterval.
    ...
    if cc <> XHCI_CC_SUCCESS then begin
        syslog.log('XHCI', 'Configure Endpoint failed, cc=');
        syslog.writeintln(cc);
        xhci_free_ring(ring^);
        kfree(void(ring));
        pop_trace;
    ...
    syslog.log('XHCI', 'Configured EP DCI=');
    syslog.writeint(dci);
    syslog.writestring(' slot=');
    syslog.writeintln(slotID);
...
{ Map xHCI completion code to USB transfer status }
...
var
    ...
    bufPhys  : uint32;
begin
    push_trace('XHCI.xhci_submit_control');
    ...
    if slotID = 0 then begin
        { Device at address 0 — this is the initial 8-byte GET_DESCRIPTOR before SET_ADDRESS.
          For xHCI, the device was already addressed in port_reset. The usbcore calls
          with address 0 first, but we already set NextAddress = slotID, so after
          the first GET_DESCRIPTOR, usbcore sets the address = slotID via SET_ADDRESS.
          We need to use the slot ID that was just assigned. Find it from NextAddress. }
        slotID := hc^.NextAddress;
        if slotID = 0 then begin
            syslog.logln('XHCI', 'No slot for address-0 device!');
            pop_trace;
            exit;
        end;
    end;
    ...
    if ring = nil then begin
        syslog.logln('XHCI', 'No EP0 ring for slot!');
        pop_trace;
        exit;
    end;
...
var
    ...
    bufPhys : uint32;
begin
    push_trace('XHCI.xhci_submit_bulk_intr');
    ...
    epIsIn := (transfer^.Direction = usbtypes.dirIn);
    ...
    if ring = nil then begin
        ...
        if ring = nil then begin
            syslog.logln('XHCI', 'Failed to configure EP for bulk/intr!');
            pop_trace;
            exit;
        end;
    end;
...
begin
    ...
    case transfer^.PipeType of
        ...
        syslog.logln('XHCI', 'Unsupported pipe type for submit.');
    end;
    pop_trace;
end;
...
begin
    ...
    syslog.writestringln(' xHCI controller(s).');
    ...
    for i := 0 to count - 1 do begin
        ...
        if mmioBase = 0 then begin
            syslog.logln('XHCI', 'Invalid MMIO base (BAR0=0), skipping.');
            continue;
        end;
        ...
        syslog.log('XHCI', 'HCI Version: ');
        syslog.writehexln(hciver);
        syslog.log('XHCI', 'CAPLENGTH=');
        syslog.writehex(capLen);
        syslog.writestring(' HCSPARAMS1=');
        syslog.writehex(hcsparams1);
        syslog.writestring(' HCSPARAMS2=');
        syslog.writehex(hcsparams2);
        syslog.writestring(' HCCPARAMS1=');
        syslog.writehexln(hccparams1);
        syslog.log('XHCI', 'DBOFF=');
        syslog.writehex(dboff);
        syslog.writestring(' RTSOFF=');
        syslog.writehexln(rtsoff);

        { Allocate private data }
        priv := PXHCI_PrivData(kalloc(sizeof(TXHCI_PrivData)));
        if priv = nil then begin
            syslog.logln('XHCI', 'Failed to allocate private data!');
            continue;
        end;
        ...
        syslog.log('XHCI', 'MaxSlots=');
        syslog.writeint(priv^.MaxSlots);
        syslog.writestring(' MaxPorts=');
        syslog.writeint(priv^.MaxPorts);
        syslog.writestring(' CtxSize=');
        syslog.writeintln(priv^.CtxSize);
        ...
        if not xhci_reset(@hc) then begin
            syslog.logln('XHCI', 'Reset failed, skipping controller.');
            kfree(void(priv));
            continue;
        end;
        ...
        if not xhci_start(@hc) then begin
            syslog.logln('XHCI', 'Start failed, skipping controller.');
            kfree(void(priv));
            continue;
        end;
...
begin
    passed := 0;
    failed := 0;
    syslog.logln('XHCI', 'Unit tests starting...');
