{
    Driver->Bus->driver.bus.usb->USBHub - driver.bus.usb Hub Class Driver.

    Implements the driver.bus.usb Hub class driver (bDeviceClass=$09) for both
...
const
    { Hub Class Feature Selectors (driver.bus.usb 2.0 spec Table 11-17) }
    ...
    { Maximum ports per hub (driver.bus.usb spec allows 255, but 15 is practical) }
    HUB_MAX_PORTS                = 15;
...
begin
    push_trace('driver.bus.usb.hub.hub_get_port_status');
...
var
    status : TUSBTransferStatus;
begin
    push_trace('driver.bus.usb.hub.hub_set_port_feature');
    status := usb_control_msg(
        dev,
...
var
    status : TUSBTransferStatus;
begin
    push_trace('driver.bus.usb.hub.hub_clear_port_feature');
    status := usb_control_msg(
        dev,
        USB_REQTYPE_DIR_OUT OR USB_REQTYPE_TYPE_CLASS OR USB_REQTYPE_REC_OTHER,
...
var
    status : TUSBTransferStatus;
begin
    push_trace('driver.bus.usb.hub.hub_get_descriptor');
    { Hub descriptors use class-specific GET_DESCRIPTOR.
      wValue = (descriptor type << 8) | descriptor index.
      USB_DESC_HUB = $29 for driver.bus.usb 2.0 hubs. }
    status := usb_control_msg(
        dev,
...
var
    ...
    loops : uint32;
begin
    push_trace('driver.bus.usb.hub.hub_port_reset');
    hub_port_reset := false;

    { Issue port reset }
    if not hub_set_port_feature(dev, port, HUB_FEAT_PORT_RESET) then begin
        io.syslog.log('USBHub', 'Port ');
        io.syslog.writeint(port);
        io.syslog.writestringln(' reset SET_FEATURE failed.');
        pop_trace;
        exit;
    end;
    ...
    if (ps.wPortStatus AND HUB_PORT_STAT_ENABLE) <> 0 then begin
        ...
        io.syslog.log('USBHub', 'Port ');
        io.syslog.writeint(port);
        io.syslog.writestringln(' reset but not enabled.');
    end;
...
begin
    push_trace('driver.bus.usb.hub.hub_configure');
    hub_configure := false;
    ...
    if depth > HUB_MAX_DEPTH then begin
        io.syslog.logln('USBHub', 'Maximum hub depth exceeded, skipping.');
        pop_trace;
        exit;
    end;

    io.syslog.log('USBHub', 'Configuring hub at address ');
    io.syslog.writeint(dev^.Address);
    io.syslog.writestring(' depth=');
    io.syslog.writeintln(depth);
    ...
    if not hub_get_descriptor(dev, @hubDesc, sizeof(TUSBHubDescriptor)) then begin
        io.syslog.logln('USBHub', 'Failed to get hub descriptor.');
        pop_trace;
        exit;
    end;

    if hubDesc.bNbrPorts = 0 then begin
        io.syslog.logln('USBHub', 'Hub reports 0 ports.');
        pop_trace;
        exit;
    end;
    ...
    io.syslog.log('USBHub', 'Hub has ');
    io.syslog.writeint(hubDesc.bNbrPorts);
    io.syslog.writestring(' port(s), PwrOn2PwrGood=');
    io.syslog.writeint(hubDesc.bPwrOn2PwrGood);
    io.syslog.writestringln('.');

    { Step 2: Allocate hub state }
    hub := PUSBHubData(kalloc(sizeof(TUSBHubData)));
    if hub = nil then begin
        io.syslog.logln('USBHub', 'Failed to allocate hub data.');
        pop_trace;
        exit;
    end;
    ...
    for port := 1 to hub^.NumPorts do begin
        ...
        if (ps.wPortStatus AND HUB_PORT_STAT_CONNECTION) <> 0 then begin
            io.syslog.log('USBHub', 'Device on port ');
    ...
    io.syslog.log('USBHub', 'Hub at address ');
    io.syslog.writeint(dev^.Address);
    io.syslog.writestringln(' configured.');
...
var
    ...
    childDev : PUSBDevice;
begin
    push_trace('driver.bus.usb.hub.hub_process_status_change');
        ...
        if (bitmap AND (1 SHL port)) <> 0 then begin
            ...
            if (ps.wPortChange AND HUB_PORT_STAT_C_CONNECTION) <> 0 then begin
                ...
                if (ps.wPortStatus AND HUB_PORT_STAT_CONNECTION) <> 0 then begin
                    ...
                    io.syslog.log('USBHub', 'New device on port ');
                    io.syslog.writeintln(port);
                    ...
                    { Device disconnected }
                    io.syslog.log('USBHub', 'Device removed from port ');
                    io.syslog.writeintln(port);
            ...
            if (ps.wPortChange AND HUB_PORT_STAT_C_OVER_CURRENT) <> 0 then begin
                ...
                io.syslog.log('USBHub', 'Overcurrent on port ');
                io.syslog.writeintln(port);
            end;
    ...
    for i := 0 to LL_Size(HubList) - 1 do begin
        ...
        if hub^.StatusXfer^.Status = tsSuccess then begin
            ...
            if hub^.StatusXfer^.Status <> tsNAK then begin
                io.syslog.log('USBHub', 'Interrupt transfer error on hub addr ');
                io.syslog.writeintln(hub^.Device^.Address);
            end;
...
{ Called by driver.mgr when a driver.bus.usb device with class $09 is found }
function load(ptr : void) : boolean;
var
    dev   : PUSBDevice;
    depth : uint8;
begin
    push_trace('driver.bus.usb.hub.load');
    load := false;
...
begin
    push_trace('driver.bus.usb.hub.init');
    io.syslog.logln('USBHub', 'INIT BEGIN.');

    HubList := LL_New(sizeof(TUSBHubData));

    { Register as a driver.bus.usb class driver matching hub devices (class $09) }
    HubID.Bus := biUSB;
    ...
    driver.mgr.register_driver('driver.bus.usb Hub Driver', @HubID, @load);

    io.syslog.logln('USBHub', 'INIT END.');
    pop_trace;
end;
...
begin
    passed := 0;
    failed := 0;
    io.syslog.logln('USBHub', 'Unit tests starting...');
