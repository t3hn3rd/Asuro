{
    Driver->Bus->driver.bus.usb->USBCore - driver.bus.usb Core Enumeration, Transfer API & HC Management.

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.bus.usb.core;
...
{ Register a host controller with the driver.bus.usb core. Called by each HC driver after init.
...
{ Register a completion hook — called by HC ISRs after processing completions.
  HID drivers (driver.hid.keyboard, driver.hid.mouse) use this to get notified without polling. }
...
{ Initialize the driver.bus.usb core. }
procedure init;
...
var
    entry : PUSBHCDriver;
begin
    push_trace('driver.bus.usb.core.register_hc');
    register_hc := nil;
    if (hc <> nil) and (HCList <> nil) then begin
        ...
        if entry <> nil then begin
            ...
            io.syslog.log('driver.bus.usb Core', 'Registered HC: ');
            io.syslog.writestringln(hc^.Name);
            register_hc := entry;
        end;
    end;
    pop_trace;
...
var
    ...
    entry : PUSBHCDriver;
begin
    push_trace('driver.bus.usb.core.unregister_hc');
    if (hc <> nil) and (HCList <> nil) then begin
        for i := 0 to LL_Size(HCList) - 1 do begin
            ...
            if (entry <> nil) and (entry^.BaseAddr = hc^.BaseAddr) and (entry^.HCType = hc^.HCType) then begin
                LL_Delete(HCList, i);
                io.syslog.log('driver.bus.usb Core', 'Unregistered HC: ');
                io.syslog.writestringln(hc^.Name);
                break;
            end;
        end;
...
var
    ...
    speed    : uint8;
begin
    push_trace('driver.bus.usb.core.scan_ports');
    if hc = nil then begin
        pop_trace;
        exit;
    end;
    io.syslog.log('driver.bus.usb Core', 'Scanning ports on HC: ');
    io.syslog.writestringln(hc^.Name);
    for port := 0 to hc^.NumPorts - 1 do begin
        if hc^.fnPortStatus <> nil then begin
            ...
            if (status AND $01) <> 0 then begin
                io.syslog.log('driver.bus.usb Core', 'Device detected on port ');
                io.syslog.writeintln(port);
...
var
    ...
    entry : PUSBHCDriver;
begin
    push_trace('driver.bus.usb.core.poll_all');
...
var
    ...
    polls    : uint32;
begin
    push_trace('driver.bus.usb.core.usb_control_msg');
...
function usb_get_descriptor(dev : PUSBDevice;
                            ...
                            wLength : uint16) : TUSBTransferStatus;
begin
    push_trace('driver.bus.usb.core.usb_get_descriptor');
...
function usb_set_address(dev : PUSBDevice; addr : uint8) : TUSBTransferStatus;
begin
    push_trace('driver.bus.usb.core.usb_set_address');
...
function usb_set_configuration(dev : PUSBDevice; config : uint8) : TUSBTransferStatus;
begin
    push_trace('driver.bus.usb.core.usb_set_configuration');
...
begin
    push_trace('driver.bus.usb.core.usb_interrupt_transfer');
...
var
    transfer : PUSBTransfer;
begin
    push_trace('driver.bus.usb.core.usb_bulk_transfer');
...
var
    ...
    ticks     : uint32;
begin
    push_trace('driver.bus.usb.core.usb_bulk_transfer_wait');
    ...
    startTick := arch.x86.bda.Counters.c32;
    while (transfer^.Status = tsInProgress) or (transfer^.Status = tsNotStarted) do begin
        if dev^.HC^.fnPoll <> nil then
            dev^.HC^.fnPoll(dev^.HC);
        if (arch.x86.bda.Counters.c32 - startTick) > ticks then begin
            kfree(void(transfer));
            usb_bulk_transfer_wait := tsTimeout;
            pop_trace;
            exit;
...
begin
    push_trace('driver.bus.usb.core.enumerate_device');
    ...
    if parentHub = nil then begin
        if hc^.fnPortReset <> nil then begin
            if not hc^.fnPortReset(hc, port) then begin
                io.syslog.logln('driver.bus.usb Core', 'Port reset failed.');
                pop_trace;
                exit;
            end;
        end;
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('driver.bus.usb Core', 'Failed to get device descriptor (8 bytes).');
        kfree(void(dev));
        pop_trace;
        exit;
    end;
    ...
    if addr > USB_MAX_DEVICES then begin
        io.syslog.logln('driver.bus.usb Core', 'No available driver.bus.usb addresses.');
        kfree(void(dev));
        pop_trace;
        exit;
    end;
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('driver.bus.usb Core', 'SET_ADDRESS failed.');
        kfree(void(dev));
        pop_trace;
        exit;
    end;
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('driver.bus.usb Core', 'Failed to get full device descriptor.');
        kfree(void(dev));
        pop_trace;
        exit;
    end;
    ...
    io.syslog.log('driver.bus.usb Core', 'Device: VID=');
    io.syslog.writehex(devDesc.idVendor);
    io.syslog.writestring(' PID=');
    io.syslog.writehex(devDesc.idProduct);
    io.syslog.writestring(' Class=');
    io.syslog.writehex(devDesc.bDeviceClass);
    io.syslog.writestring(' Addr=');
    io.syslog.writeintln(addr);
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('driver.bus.usb Core', 'Failed to get config descriptor.');
        kfree(void(dev));
        pop_trace;
        exit;
    end;
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('driver.bus.usb Core', 'Failed to get full config descriptor.');
        kfree(void(cfgBuf));
        kfree(void(dev));
        pop_trace;
        exit;
    ...
    if status <> tsSuccess then begin
        io.syslog.logln('driver.bus.usb Core', 'SET_CONFIGURATION failed.');
        kfree(void(cfgBuf));
        kfree(void(dev));
        pop_trace;
        exit;
    ...
    { Step 9: Register with driver.mgr using biUSB }
    ...
    driver.mgr.register_device('driver.bus.usb Device', DevID, void(dev));
    kfree(void(DevID));
    ...
    io.syslog.log('driver.bus.usb Core', 'Device enumerated at address ');
    io.syslog.writeintln(addr);
...
var
    epAddr : uint16;
begin
    push_trace('driver.bus.usb.core.usb_clear_halt');
    ...
    epAddr := ep^.Address;
    if ep^.Direction = driver.bus.usb.types.dirIn then
        epAddr := epAddr OR $80;
    ...
    if usb_clear_halt = tsSuccess then begin
        ep^.Toggle := 0;
        io.syslog.log('driver.bus.usb Core', 'Cleared halt on EP');
        io.syslog.writeintln(ep^.Address);
    end else begin
        io.syslog.log('driver.bus.usb Core', 'Failed to clear halt on EP');
        io.syslog.writeintln(ep^.Address);
    end;
    pop_trace;
end;
...
var
    ...
    dptr : PUSBDevice;
begin
    push_trace('driver.bus.usb.core.usb_remove_device');
    ...
    io.syslog.log('driver.bus.usb Core', 'Removed device at address ');
    io.syslog.writeintln(dev^.Address);
...
var
    ...
    found  : boolean;
begin
    push_trace('driver.bus.usb.core.usb_rescan_port');
    ...
    if (status AND $01) <> 0 then begin
        ...
        if not found then begin
            io.syslog.log('driver.bus.usb Core', 'Hotplug: new device on port ');
            io.syslog.writeintln(port);
        ...
        if found and (dev <> nil) and (dev^.Address > 0) then begin
            io.syslog.log('driver.bus.usb Core', 'Hotplug: device removed from port ');
            io.syslog.writeintln(port);
...
{ ========================= driver.bus.usb Terminal Command ========================= }
...
begin
    ...
    io.stdio.bufWriteStrLn(stdout_buf, 'driver.bus.usb Subsystem Information');
    io.stdio.bufWriteStrLn(stdout_buf, '=========================');
    io.stdio.bufWriteStr(stdout_buf, 'Host Controllers: ');
    io.stdio.bufWriteIntLn(stdout_buf, hcCount);
    ...
    for i := 0 to hcCount - 1 do begin
        ...
        { HC header }
        case hc^.HCType of
            USB_HC_UHCI: hcName := 'driver.bus.usb.uhci';
            USB_HC_OHCI: hcName := 'driver.bus.usb.ohci';
            USB_HC_EHCI: hcName := 'driver.bus.usb.ehci';
            USB_HC_XHCI: hcName := 'driver.bus.usb.xhci';
        else
            hcName := '????';
        end;

        io.stdio.bufWriteStr(stdout_buf, 'HC ');
        io.stdio.bufWriteInt(stdout_buf, i);
        io.stdio.bufWriteStr(stdout_buf, ': ');
        io.stdio.bufWriteStr(stdout_buf, hcName);
        io.stdio.bufWriteStr(stdout_buf, ' @ 0x');
        io.stdio.bufWriteHex(stdout_buf, hc^.BaseAddr);
        io.stdio.bufWriteStr(stdout_buf, ' (IRQ ');
        io.stdio.bufWriteInt(stdout_buf, hc^.PCIDev.interrupt_line);
        io.stdio.bufWriteStr(stdout_buf, ', ');
        io.stdio.bufWriteInt(stdout_buf, hc^.NumPorts);
        io.stdio.bufWriteStrLn(stdout_buf, ' ports)');
        ...
        if devCount = 0 then begin
            io.stdio.bufWriteStrLn(stdout_buf, '  (no devices)');
        end else begin
            for j := 0 to devCount - 1 do begin
                dev := PUSBDevice(puint32(LL_Get(hc^.Devices, j))^);
                if dev = nil then continue;

                io.stdio.bufWriteStr(stdout_buf, '  Dev ');
                io.stdio.bufWriteInt(stdout_buf, dev^.Address);
                io.stdio.bufWriteStr(stdout_buf, ': VID=0x');
                io.stdio.bufWriteHex(stdout_buf, dev^.DevDesc.idVendor);
                io.stdio.bufWriteStr(stdout_buf, ' PID=0x');
                io.stdio.bufWriteHex(stdout_buf, dev^.DevDesc.idProduct);

                { Speed }
                io.stdio.bufWriteStr(stdout_buf, ' [');
                case dev^.Speed of
                    USB_SPEED_LOW:   io.stdio.bufWriteStr(stdout_buf, 'Low');
                    USB_SPEED_FULL:  io.stdio.bufWriteStr(stdout_buf, 'Full');
                    USB_SPEED_HIGH:  io.stdio.bufWriteStr(stdout_buf, 'High');
                    USB_SPEED_SUPER: io.stdio.bufWriteStr(stdout_buf, 'Super');
                else
                    io.stdio.bufWriteStr(stdout_buf, '?');
                end;
                io.stdio.bufWriteStrLn(stdout_buf, ' Speed]');

                { Class info }
                io.stdio.bufWriteStr(stdout_buf, '    Class=0x');
                io.stdio.bufWriteHex(stdout_buf, dev^.DevDesc.bDeviceClass);
                io.stdio.bufWriteStr(stdout_buf, ' Sub=0x');
                io.stdio.bufWriteHex(stdout_buf, dev^.DevDesc.bDeviceSubClass);
                io.stdio.bufWriteStr(stdout_buf, ' Proto=0x');
                io.stdio.bufWriteHex(stdout_buf, dev^.DevDesc.bDeviceProtocol);
                io.stdio.bufWriteStr(stdout_buf, ' MaxPkt0=');
                io.stdio.bufWriteIntLn(stdout_buf, dev^.MaxPacket0);

                { Endpoints }
                if dev^.NumEndpoints > 0 then begin
                    io.stdio.bufWriteStr(stdout_buf, '    Endpoints: ');
                    io.stdio.bufWriteIntLn(stdout_buf, dev^.NumEndpoints);
                    for k := 0 to dev^.NumEndpoints - 1 do begin
                        io.stdio.bufWriteStr(stdout_buf, '      EP');
                        io.stdio.bufWriteInt(stdout_buf, dev^.Endpoints[k].Address);
                        if dev^.Endpoints[k].Direction = driver.bus.usb.types.dirIn then
                            io.stdio.bufWriteStr(stdout_buf, ' IN  ')
                        else
                            io.stdio.bufWriteStr(stdout_buf, ' OUT ');
                        case dev^.Endpoints[k].PipeType of
                            ptControl:     io.stdio.bufWriteStr(stdout_buf, 'Control');
                            ptIsochronous: io.stdio.bufWriteStr(stdout_buf, 'Isoch');
                            ptBulk:        io.stdio.bufWriteStr(stdout_buf, 'Bulk');
                            ptInterrupt:   io.stdio.bufWriteStr(stdout_buf, 'Interrupt');
                        end;
                        io.stdio.bufWriteStr(stdout_buf, ' MaxPkt=');
                        io.stdio.bufWriteIntLn(stdout_buf, dev^.Endpoints[k].MaxPacket);
                    end;
                end;
            end;
        end;
    ...
    io.stdio.bufWriteStr(stdout_buf, 'Total devices: ');
    io.stdio.bufWriteIntLn(stdout_buf, totalDevs);
end;
...
var
    i : uint32;
begin
    push_trace('driver.bus.usb.core.init');
    io.syslog.logln('driver.bus.usb Core', 'INIT BEGIN.');
    HCList := LL_New(sizeof(TUSBHCDriver));
    CompletionHookCount := 0;
    for i := 0 to MAX_COMPLETION_HOOKS - 1 do
        CompletionHooks[i] := nil;
    io.syslog.logln('driver.bus.usb Core', 'INIT END.');
    pop_trace;
end;
...
begin
    passed := 0;
    failed := 0;
    io.syslog.logln('USBCORE', 'Unit tests starting...');
    ...
    PUint8(uint32(cfgBuf) + 16)^ := $01; { bInterfaceProtocol (driver.hid.keyboard) }
