{
    Driver->Bus->driver.bus.usb->driver.bus.usb.ehci - Enhanced Host Controller Interface Driver.

    Implements the driver.bus.usb.ehci (driver.bus.usb 2.0) host controller for MMIO-based controllers.
    driver.bus.usb.ehci uses Queue Heads (QH), queue Transfer Descriptors (qTD), and a
    Periodic Frame List for scheduling high-speed transfers.

    driver.bus.usb.ehci also handles companion controller (driver.bus.usb.uhci/driver.bus.usb.ohci) routing for
    full/low-speed devices via the CONFIGFLAG and PORTSC port-owner bits.

    @author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.bus.usb.ehci;
...
{ ========================= driver.bus.usb.ehci Capability Register Offsets ========================= }
...
{ ========================= driver.bus.usb.ehci Operational Register Offsets ========================= }

    { Operational Registers (at MMIO Base + CAPLENGTH) }
    EHCI_OP_USBCMD             = $00;  { driver.bus.usb Command }
    EHCI_OP_USBSTS             = $04;  { driver.bus.usb Status }
    EHCI_OP_USBINTR            = $08;  { driver.bus.usb Interrupt Enable }
...
{ ========================= USBSTS Bits ========================= }
    ...
    EHCI_STS_USBINT            = $00000001; { driver.bus.usb Interrupt (transfer complete) }
    EHCI_STS_USBERRINT         = $00000002; { driver.bus.usb Error Interrupt }
...
{ ========================= HCCPARAMS Bits ========================= }
    ...
    EHCI_HCC_EECP_MASK         = $0000FF00; { driver.bus.usb.ehci Extended Capabilities Pointer }
    EHCI_HCC_EECP_SHIFT        = 8;

{ ========================= CONFIGFLAG ========================= }

    EHCI_CF_FLAG               = $00000001; { Configure Flag: 1=route all ports to driver.bus.usb.ehci }
...
{ ========================= driver.bus.usb.ehci Data Structures ========================= }
...
type
    ...
    { Private data for an driver.bus.usb.ehci host controller instance }
...
{ Map qTD status bits to driver.bus.usb transfer status }
...
{ Fill qTD buffer pointers for a given virtual buffer.
  driver.bus.usb.ehci qTDs use up to 5 page pointers (each 4KB-aligned) plus
...
var
    ...
    loops  : uint32;
begin
    push_trace('driver.bus.usb.ehci.ehci_bios_handoff');
    ...
    { Read capability ID at EECP offset in driver.bus.pci config }
    driver.bus.pci.requestConfig(pciDev.bus, pciDev.slot, pciDev.func, row);
    cap := inl($CFC);
    if (cap AND $FF) <> 1 then begin
        { Not a driver.bus.usb Legacy Support capability }
        pop_trace;
        exit;
    end;
    ...
    if (legsup AND $00010000) = 0 then begin
        io.syslog.logln('driver.bus.usb.ehci', 'BIOS does not own controller.');
        pop_trace;
        exit;
    end;

    { Request ownership: set OS Owned Semaphore (bit 24) }
    driver.bus.pci.writeConfig(pciDev.bus, pciDev.slot, pciDev.func, row,
                     legsup OR $01000000);
    ...
    while loops < 200000 do begin
        ...
        legsup := inl($CFC);
        if (legsup AND $00010000) = 0 then begin
            io.syslog.logln('driver.bus.usb.ehci', 'BIOS handoff complete.');
            pop_trace;
            exit;
        end;
        inc(loops);
    ...
    io.syslog.logln('driver.bus.usb.ehci', 'BIOS handoff timeout, forcing ownership.');
    driver.bus.pci.writeConfig(pciDev.bus, pciDev.slot, pciDev.func, row,
                     (legsup AND (NOT uint32($00010000))) OR $01000000);

    { Clear legacy support control/status }
    if (eecp + 4) < 256 then
        driver.bus.pci.writeConfig(pciDev.bus, pciDev.slot, pciDev.func, row + 1, 0);
...
var
    ...
    cmd    : uint32;
begin
    push_trace('driver.bus.usb.ehci.ehci_reset');
    ehci_reset := false;
    ...
    if (ehci_readl(opbase, EHCI_OP_USBSTS) AND EHCI_STS_HCHALTED) = 0 then begin
        io.syslog.logln('driver.bus.usb.ehci', 'Controller did not halt!');
        pop_trace;
        exit;
    end;
    ...
    if (ehci_readl(opbase, EHCI_OP_USBCMD) AND EHCI_CMD_HCRESET) <> 0 then begin
        io.syslog.logln('driver.bus.usb.ehci', 'HC reset timeout!');
        pop_trace;
        exit;
    end;
    ...
    io.syslog.logln('driver.bus.usb.ehci', 'HC reset complete.');
    ehci_reset := true;
    pop_trace;
end;
...
var
    ...
    intrQH   : PEHCI_QH;
begin
    push_trace('driver.bus.usb.ehci.ehci_setup_schedule');
    priv := PEHCI_PrivData(hc^.PrivData);
    ...
    if priv^.FrameList = nil then begin
        io.syslog.logln('driver.bus.usb.ehci', 'Failed to allocate frame list!');
        pop_trace;
        exit;
    end;
    ...
    if intrQH = nil then begin
        io.syslog.logln('driver.bus.usb.ehci', 'Failed to allocate interrupt QH!');
        pop_trace;
        exit;
    end;
    priv^.IntrQH := intrQH;
    ...
    if asyncQH = nil then begin
        io.syslog.logln('driver.bus.usb.ehci', 'Failed to allocate async QH!');
        pop_trace;
        exit;
    end;
    ...
    io.syslog.logln('driver.bus.usb.ehci', 'Schedule configured.');
    pop_trace;
end;
...
var
    ...
    port   : uint32;
begin
    push_trace('driver.bus.usb.ehci.ehci_start');
    ehci_start := false;
    if (hc = nil) or (hc^.PrivData = nil) then begin
        pop_trace;
        exit;
    ...
    { Set CONFIGFLAG to route all ports to driver.bus.usb.ehci }
    ...
    if (ehci_readl(opbase, EHCI_OP_USBSTS) AND EHCI_STS_HCHALTED) <> 0 then begin
        io.syslog.logln('driver.bus.usb.ehci', 'Controller failed to start (still halted).');
        pop_trace;
        exit;
    end;
    ...
    io.syslog.logln('driver.bus.usb.ehci', 'Controller started (operational).');
    ehci_start := true;
    pop_trace;
end;
...
var
    ...
    cmd    : uint32;
begin
    push_trace('driver.bus.usb.ehci.ehci_stop');
    if (hc <> nil) and (hc^.PrivData <> nil) then begin
        ...
        cmd := cmd AND (NOT EHCI_CMD_PSE);
        ehci_writel(opbase, EHCI_OP_USBCMD, cmd);
        io.syslog.logln('driver.bus.usb.ehci', 'Controller stopped.');
    end;
    pop_trace;
end;
...
var
    priv : PEHCI_PrivData;
begin
    push_trace('driver.bus.usb.ehci.ehci_port_status');
    ehci_port_status := 0;
    if (hc = nil) or (hc^.PrivData = nil) then begin
        pop_trace;
        exit;
...
var
    ...
    loops  : uint32;
begin
    push_trace('driver.bus.usb.ehci.ehci_port_reset');
    ...
    if (status AND EHCI_PORTSC_LS_MASK) = EHCI_PORTSC_LS_KSTATE then begin
        io.syslog.log('driver.bus.usb.ehci', 'Port ');
        io.syslog.writeint(port);
        io.syslog.writestringln(' low-speed device -> companion.');
    ...
    { Clear the enable bit before reset (per driver.bus.usb.ehci spec 4.2.2) }
    status := ehci_readl(opbase, regOfs);
    status := status AND (NOT EHCI_PORTSC_WC_BITS);
    ...
    { Hold reset for ~50ms (driver.bus.usb 2.0 spec 7.1.7.5) }
    loops := 0;
    ...
    if (status AND EHCI_PORTSC_PE) = 0 then begin
        ...
        io.syslog.log('driver.bus.usb.ehci', 'Port ');
        io.syslog.writeint(port);
        io.syslog.writestringln(' not enabled -> companion.');
        status := status AND (NOT EHCI_PORTSC_WC_BITS);
        ehci_writel(opbase, regOfs, status OR EHCI_PORTSC_PO);
        pop_trace;
        exit;
    ...
    io.syslog.log('driver.bus.usb.ehci', 'Port ');
    io.syslog.writeint(port);
    io.syslog.writestringln(' reset and enabled (high-speed).');
...
var
    ...
    isCtrlEP : boolean;
begin
    push_trace('driver.bus.usb.ehci.ehci_submit_control');
...
var
    ...
    pid      : uint32;
begin
    push_trace('driver.bus.usb.ehci.ehci_submit_async');
...
begin
    push_trace('driver.bus.usb.ehci.ehci_submit');
    ehci_submit := false;
    ...
    case transfer^.PipeType of
        ...
        io.syslog.logln('driver.bus.usb.ehci', 'Unsupported pipe type for submit.');
    end;
    pop_trace;
end;
...
begin
    for i := 0 to EHCIInstanceCount - 1 do begin
        ...
        { Process completions (walks QH core.ds.lists) }
        ehci_poll(hc);
...
{ Enable hardware interrupts: driver.bus.usb INT, driver.bus.usb Error INT, Port Change, Host System Error. }
...
begin
    ...
    io.syslog.logln('driver.bus.usb.ehci', 'Hardware interrupts enabled.');
end;
...
begin
    ...
    if (usbsts AND EHCI_STS_HSE) <> 0 then begin
        io.syslog.logln('driver.bus.usb.ehci', 'Host System Error detected!');
    end;
...
var
    ...
    hciver   : uint16;
begin
    push_trace('driver.bus.usb.ehci.load');
    load := false;
    EHCIInstanceCount := 0;

    devices := driver.bus.pci.getDeviceInfo($0C, $03, $20, count);
    io.syslog.log('driver.bus.usb.ehci', 'Found ');
    io.syslog.writeint(count);
    io.syslog.writestringln(' driver.bus.usb.ehci controller(s).');
    ...
    for i := 0 to count - 1 do begin
        io.syslog.log('driver.bus.usb.ehci', 'Controller[');
        ...
        if mmioBase = 0 then begin
            io.syslog.logln('driver.bus.usb.ehci', 'Invalid MMIO base (BAR0=0), skipping.');
            continue;
        end;
        ...
        io.syslog.log('driver.bus.usb.ehci', 'HCI Version: ');
        io.syslog.writehexln(hciver);
        io.syslog.log('driver.bus.usb.ehci', 'CAPLENGTH=');
        io.syslog.writehex(capLen);
        io.syslog.writestring(' HCSPARAMS=');
        io.syslog.writehex(hcsparams);
        io.syslog.writestring(' HCCPARAMS=');
        io.syslog.writehexln(hccparams);

        { Allocate private data }
        priv := PEHCI_PrivData(kalloc(sizeof(TEHCI_PrivData)));
        if priv = nil then begin
            io.syslog.logln('driver.bus.usb.ehci', 'Failed to allocate private data!');
            continue;
        end;
        ...
        io.syslog.log('driver.bus.usb.ehci', 'Ports=');
        io.syslog.writeint(priv^.NumPorts);
        io.syslog.writestring(' CC=');
        io.syslog.writeint(priv^.NumCC);
        io.syslog.writestring(' PPC=');
        io.syslog.writeintln(priv^.PortsPerCC);
        ...
        if not ehci_reset(@hc) then begin
            io.syslog.logln('driver.bus.usb.ehci', 'Reset failed, skipping controller.');
            kfree(void(priv));
            continue;
        end;
        ...
        if not ehci_start(@hc) then begin
            io.syslog.logln('driver.bus.usb.ehci', 'Start failed, skipping controller.');
        ...
        if hcEntry <> nil then begin
            ...
            io.syslog.log('driver.bus.usb.ehci', 'Registering ISR on IRQ ');
            io.syslog.writeintln(devices[i].interrupt_line);
            arch.x86.isr.mgr.registerISR(32 + devices[i].interrupt_line, @ehci_isr);
            ...
            io.syslog.logln('driver.bus.usb.ehci', 'Failed to register HC with driver.bus.usb core.');

        io.syslog.logln('driver.bus.usb.ehci', 'Controller initialized and registered.');
    end;
...
var
    ...
    begin
        if condition then begin
            ...
            io.syslog.logln('driver.bus.usb.ehci', msg);
            kfree(void(msg));
        end;
    end;
...
begin
    passed := 0;
    failed := 0;
    io.syslog.logln('driver.bus.usb.ehci', 'Unit tests starting...');
