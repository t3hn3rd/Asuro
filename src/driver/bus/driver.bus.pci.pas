...
{ 
	Driver->Bus->driver.bus.pci - Peripheral Component Interconnect Driver.
...
begin
    push_trace('driver.bus.pci.load');
    io.syslog.logln('driver.bus.pci', 'Scanning Bus: 0');
    scanBus(0);
    //while unscanned busses scan busses
    current_bus := 1;
    while true do begin
        if current_bus < bus_count then begin
            io.syslog.log('driver.bus.pci', 'Scanning Bus: ');
            io.syslog.writeintln(bus_count);
            scanBus(current_bus);
...
begin
    push_trace('driver.bus.pci.init');
    io.syslog.logln('driver.bus.pci','INIT BEGIN.');
    DevID.Bus:= biUnknown;
    DevID.id0:= 0;
    DevID.id1:= 0;
    DevID.id2:= 0;
    DevID.id3:= 0;
    DevID.ex:= nil;
    driver.mgr.register_driver_ex('driver.bus.pci Driver', @DevID, @load, true);
    io.syslog.logln('driver.bus.pci', 'INIT END.');
    pop_trace;
end;
...
begin
    push_trace('driver.bus.pci.scanBus');
...
begin
    push_trace('driver.bus.pci.writeConfig');
    addr := ($1 shl 31);
...
begin
    push_trace('driver.bus.pci.setBusMaster');
...
begin
    push_trace('driver.bus.pci.requestConfig');
    addr := ($1 shl 31);
    addr := addr or (bus shl 16);
    addr := addr or ((slot) shl 11);
    addr := addr or ((func) shl 8);
...
begin
    push_trace('driver.bus.pci.loadBusConfig');
...
begin
    push_trace('driver.bus.pci.loadDeviceConfig');
    ...
    if device.vendor_id <> $FFFF then begin
        ...
        if device.header_type and $7 = 0 then begin 
            ...
            io.syslog.log('driver.bus.pci', 'Found Device: ');
            io.syslog.writehex(device.header_type);
            io.syslog.writestring('  ');
            io.syslog.writehex(device.device_id);        
            io.syslog.writestring('  ');        
            io.syslog.writehex(device.class_code);        
            io.syslog.writestring('  ');
            io.syslog.writehex(device.subclass_class);        
            io.syslog.writestring('  ');
            io.syslog.writehexln(device.prog_if);

            devices[device_count] := device;
            device_count := device_count + 1;

            driver.mgr.register_device('driver.bus.pci Device', DevID, @device);
            kfree(void(DevID));
        end else begin
            loadBusConfig(bus, slot, func, device);
        end;
        //TODO implement other core.types?
    end;   
    //if device.class_code = 1 then driver.storage.ctl.ide.ata.init(device);   
    pop_trace;
end;
...
begin
    push_trace('driver.bus.pci.getDeviceInfo');
    count := 0;
...
var
    ...
    cmd : uint32;
begin
    push_trace('driver.bus.pci.enableDevice');
