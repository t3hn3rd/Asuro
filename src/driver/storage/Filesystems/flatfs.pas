...
begin
    ...
    storagemanager.storage_write(volume^.device, start, 1, PuInt32(info));// what happens if buffer is smaller than 512?
    ...
    //write file table
    storagemanager.storage_write(volume^.device, start + 1, ((sizeof(TFile_Entry) * info^.fileCount) div 512), puint32(entryTable));
...
begin
    ...
    storagemanager.storage_read(volume^.device, volume^.sectorStart + 1, (sizeof(TFile_Entry) * fileCount) div 512, PuInt32(fileEntries));
    ...
    if status^ = 0 then begin
        for i := 0 to fileCount - 1 do begin
            ...
            if fileEntry^.attribues = 0 then begin
                ...
                storagemanager.storage_read(volume^.device, volume^.sectorStart + 1 + position, 1, buffer);
                ...
                storagemanager.storage_write(volume^.device, volume^.sectorStart + 1 + position, 1, buffer);
...
begin
    ...
    storagemanager.storage_read(volume^.device, volume^.sectorStart + 1, sizeof(TFile_Entry) * fileCount div 512, puint32(fileEntrys));
...
begin
    ...
    //write file table
    storagemanager.storage_write(volume^.device, volume^.sectorStart + 1 , 1, puint32(@fileEntries[position]));

    //write file
    storagemanager.storage_write(volume^.device, fileEntries[p].start, size, data);
...
begin
    ...
    //read file
    data := Kalloc(size);
    storagemanager.storage_read(volume^.device, fileEntries[p].start, size, data);
...
begin
    ...
    //write file
    storagemanager.storage_write(volume^.device, fileEntries[p].start, fileEntries[p].size, zeroBuffer);
...
var
    ...
    slashArray  : array[0..1] of char;
begin
    push_trace('flatfs.readDirectoryEntries');
    fileCount := 1000;
    slashArray[0] := '/';
    slashArray[1] := char(0);
