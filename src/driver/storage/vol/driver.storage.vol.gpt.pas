...
//  limitations under the License.
{
    ...
    Provides core.types and helpers for reading/validating GPT headers and
...
var
    ...
    i        : uint32;
begin
    push_trace('driver.storage.vol.gpt.validate_header');
    ...
    for i := 0 to 7 do begin
        if header^.Signature[i] <> GPT_SIGNATURE[i] then begin
            io.syslog.logln('GPT', 'Signature mismatch.');
            exit;
        end;
    end;
    ...
    calcCRC := core.enc.crc.CRC32(puint8(header), header^.HeaderSize);
    header^.HeaderCRC32 := savedCRC;

    if calcCRC <> savedCRC then begin
        io.syslog.logln('GPT', 'Header CRC32 mismatch.');
        exit;
    end;
...
var
    ...
    calcCRC   : uint32;
begin
    push_trace('driver.storage.vol.gpt.validate_entries');
    ...
    calcCRC := core.enc.crc.CRC32(entryBuf, totalSize);
    if calcCRC <> header^.PartArrayCRC32 then begin
        io.syslog.logln('GPT', 'Partition array CRC32 mismatch.');
        exit;
    end;
...
begin
    ...
    err := driver.storage.mgr.storage_read(device, lba, 1, buf);
...
var
    ...
    backupLBA  : uint32;
begin
    push_trace('driver.storage.vol.gpt.read_header');
    gpt_read_header := nil;
    ...
    io.syslog.logln('GPT', 'Primary header invalid, trying backup.');
...
var
    ...
    err          : TError;
begin
    push_trace('driver.storage.vol.gpt.read_entries');
    ...
    err := driver.storage.mgr.storage_read(device, header^.PartEntryLBA, sectorsNeeded, buf);
    if err <> eNone then begin
        kfree(buf);
        exit;
    end;
    ...
    if not gpt_validate_entries(header, puint8(buf)) then begin
        io.syslog.logln('GPT', 'Partition entry array CRC mismatch.');
        kfree(buf);
        exit;
    end;
