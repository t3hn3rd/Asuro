...
unit driver.storage.fs.iso9660;
...
begin
    ...
    driver.storage.mgr.storage_read(device, lba, count, buf);
    readSectors := buf;
end;
...
begin
    ...
    { Guard: empty core.strings are equal, and uint32 la-1 would underflow to MaxUint32 }
    if la = 0 then begin
...
begin
    ...
    curLBA := rootLBA(pvd);
    curLen := rootLen(pvd);
    io.syslog.writestring('[driver.storage.fs.iso9660] resolvePath: path='); io.syslog.writestringln(path);
    ...
    while i <= pathLen do begin
        ...
        if iters > 256 then begin
            io.syslog.writestring('[driver.storage.fs.iso9660] resolvePath: SAFETY BREAK iters='); io.syslog.writeintln(integer(iters));
            exit;
        end;
...
begin
    ...
    io.syslog.writestring('[driver.storage.fs.iso9660] readDirEntries: dirLBA='); io.syslog.writehex(dirLBA);
    io.syslog.writestring(' dirLen='); io.syslog.writehexln(dirLen);
    buf := readExtent(volume^.device, dirLBA, dirLen);
    if buf = nil then begin
        io.syslog.writestringln('[driver.storage.fs.iso9660] readDirEntries: readExtent returned nil');
        status^ := ord(eUnknown);
        readDirectoryEntries := entries;
        exit;
    end;
    io.syslog.writestringln('[driver.storage.fs.iso9660] readDirEntries: entering parse loop');
    offset := 0;
    iters  := 0;
    while offset < dirLen do begin
        ...
        if iters > 65536 then begin
            io.syslog.writestring('[driver.storage.fs.iso9660] SAFETY BREAK at offset='); io.syslog.writehex(offset);
            io.syslog.writestring(' recLen='); io.syslog.writeintln(integer(rec^.recLen));
            break;
        end;
        ...
        if rec^.recLen < 34 then begin
            io.syslog.writestring('[driver.storage.fs.iso9660] readDirEntries: recLen<34 break at offset='); io.syslog.writehexln(offset);
            break;
        end;
    ...
    kfree(buf);
    io.syslog.writestring('[driver.storage.fs.iso9660] readDirEntries: done, entries='); io.syslog.writeintln(integer(LL_Size(entries)));
    readDirectoryEntries := entries;
end;
...
procedure init();
begin
    push_trace('driver.storage.fs.iso9660.init()');
