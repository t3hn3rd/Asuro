...
unit driver.storage.fs.fat32.old;
...
procedure FAT32LegacyReleaseVolumeCtx(volume : PStorage_Volume);
...
function readDirectoryGen(volume : PStorage_volume; directory : pchar; status : puint32) : PLinkedListBase;
procedure writeDirectoryGen(volume : PStorage_volume; directory : pchar; dirName : pchar; attributes : uint32; statusOut : puint32);
...
begin
    ...
    ctx^.Cache.FatStart := volume^.sectorStart + br^.rsvSectors;
...
procedure fatCacheFlush(cache : PFATCache; volume : PStorage_Volume); forward;
...
begin
    if volume = nil then exit;
    ctx := PFATVolumeCtx(volume^.fsPrivate);
    if ctx = nil then exit;

    volume^.fsPrivate := nil;
    fatCacheFlush(@ctx^.Cache, volume);

    if ctx^.Worker <> nil then begin
        proc.mgr.terminate(ctx^.Worker^.ProcessID);
        exit;
    end;

    if ctx^.Cache.BootRecord <> nil then
        kfree(puint32(ctx^.Cache.BootRecord));
    if ctx^.Cache.Data <> nil then
        kfree(ctx^.Cache.Data);
    if ctx^.Cache.EvictBuf <> nil then
        kfree(ctx^.Cache.EvictBuf);
    if ctx^.TransferPoolBuf <> nil then
        kfree(void(ctx^.TransferPoolBuf));
    kfree(puint32(ctx));
end;
...
begin
    fatDataStartLBA := volume^.sectorStart + bootRecord^.rsvSectors + bootRecord^.FATSize;
end;
...
begin
    ...
    for i:=0 to LL_size(clusters) - 1 do begin
        sectorLocation:= clusterToLBA(dataStart, bootRecord^.spc, puint32(LL_Get(clusters, i))^);
        bufferI:= puint32(uint32(buffer) + uint32(i * bootRecord^.spc * bootRecord^.sectorSize));
        driver.storage.mgr.storage_read(volume^.device, sectorLocation, bootRecord^.spc, bufferI);
    end;
...
begin
    ...
    if status^ = ord(eNone) then begin
        ...
        driver.storage.mgr.storage_read(volume^.device,
            clusterToLBA(dataStart, bootRecord^.spc, parentCluster) + sectorLocation, 1, buffer);
...
begin
    ...
    ctx^.FATStart := start + 32;   { boot sector included in reserved sectors }
...
begin
    ...
    { Write the boot sector signature marker at bytes 510-511 }
    puint8(uint32(buffer) + 510)^ := $55;
    puint8(uint32(buffer) + 511)^ := $AA;

    driver.storage.mgr.storage_write(disk, start, 1, puint32(buffer));
    io.syslog.logln('FAT32', 'create_volume (sync): boot sector written');

    fatStart:= start + bootRecord^.rsvSectors;
...
begin
    ...
    for i := 0 to 4 do begin
        if bootRecord^.identString[i] <> 'FAT32'[i + 1] then begin
            fatNameOk := false;
            break;
        end;
    end;
    ...
    if bootSectorHasSignature(buffer) and
       (bootRecord^.bsignature = $29) and
       fatNameOk and
       (bootRecord^.sectorSize = volume^.device^.sectorSize) and
       (bootRecord^.sectorSize >= 512) and
       (spc <> 0) and
       ((spc and (spc - 1)) = 0) and
       (bootRecord^.rsvSectors > 0) and
       (bootRecord^.FATSize > 0) and
       (bootRecord^.numFats > 0) and
       (bootRecord^.rootCluster >= 2) then begin
...
begin
    ...
    { Read from sector 0 to check for FAT32 boot record }
    ...
    if bootSectorHasSignature(buffer) and (PBootRecord(buffer)^.bsignature = $29) then begin
...
procedure init();
begin
    { Legacy reference unit only. The rewritten public FAT32 unit handles
      filesystem registration and callback selection. }
end;
