...
{ 
	Drivers->Storage->driver.storage.ctl.ide - driver.storage.ctl.ide Driver.

    NOT FINSIHED NEEDS WORK
	
	@author(Aaron Hance <ah@aaronhance.me>)
}
unit driver.storage.ctl.ide;
...
{
    Disable interrupts on the driver.storage.ctl.ide bus
...
{ 
    Wait for the device to be ready on the driver.storage.ctl.ide bus
...
{ 
    Select the active device on the driver.storage.ctl.ide bus 
...
{ 
    Get the status of the device on the driver.storage.ctl.ide bus

    @param(device The device to get the status of)
    @returns(@TIDE_Device The status of the device on the driver.storage.ctl.ide bus)
}
function get_status(var device : TIDE_Device) : TIDE_Status;
var
    status : TIDE_Status;
...
begin
    ...
    if status.ERROR then begin
        ...
        console.writestringln('[driver.storage.ctl.ide] ERROR detected!');
        console.writestring('[driver.storage.ctl.ide] ERROR REGISTER: ');
        console.writebin8ln(errorReg);

        if (errorReg and $04) <> 0 then console.writestringln('[driver.storage.ctl.ide] ERROR: Aborted Command');
        if (errorReg and $10) <> 0 then console.writestringln('[driver.storage.ctl.ide] ERROR: ID Not Found');
        if (errorReg and $40) <> 0 then console.writestringln('[driver.storage.ctl.ide] ERROR: Uncorrectable Data');
        if (errorReg and $80) <> 0 then console.writestringln('[driver.storage.ctl.ide] ERROR: Bad Block');
...
{ 
    Check if the device is present on the driver.storage.ctl.ide bus
...
// }
    ...
    console.writestring('[driver.storage.ctl.ide] (check_device_type) SEC: ');
    console.writehexln(sec);
    console.writestring('[driver.storage.ctl.ide] (check_device_type) LBA0: ');
    console.writehexln(lba0);
    console.writestring('[driver.storage.ctl.ide] (check_device_type) LBA1: ');
    console.writehexln(lba1);
    console.writestring('[driver.storage.ctl.ide] (check_device_type) LBA2: ');
...
{ 
    Load the device on the driver.storage.ctl.ide bus
...
begin
    ...
    if (is_device_present(device) and success) then begin
        console.writestringln('[driver.storage.ctl.ide] (load_device) Device is present');

        if (device.isATAPI) then begin
            console.writestringln('[driver.storage.ctl.ide] (load_device) Device is ATAPI');
            ...
            console.writestringln('[driver.storage.ctl.ide] (load_device) Device is ATA');
            success:= driver.storage.ctl.ide.ata.identify_device(device);

            //check if atapi device using info from identify_device
            console.writestringln('[driver.storage.ctl.ide] (load_device) Device info: ');
        ...
        if not success then begin
            console.writestringln('[driver.storage.ctl.ide] (load_device) Error identifying device'); //todo 
            device.exists := false;
            exit;
        end;
        ...
        if (device.isATAPI) then begin
            ...
            console.writestringln('[driver.storage.ctl.ide] (load_device) First sector of ATAPI device: ');
            ...
            // driver.storage.ctl.ide.ata.write_pio28(device, 10, 2, puint16(buffer));

            // //read the first sector
            // memset(uint32(buffer), 0, 512);
            // driver.storage.ctl.ide.ata.read_pio28(device, 10, 1, puint16(buffer));
        ...
        console.writestringln('[driver.storage.ctl.ide] (load_device) Device is not present');
        device.exists := false;
    end;
...
begin
    push_trace('ide.load()');
    console.writestringln('[driver.storage.ctl.ide] (load) Loading driver.storage.ctl.ide Devices');
    console.redrawWindows();
    registerISR(14, @atapi.ide_irq);
    registerISR(15, @atapi.ide_irq);
    console.writestringln('[driver.storage.ctl.ide] (load) Loading driver.storage.ctl.ide Devices 2');
    console.redrawWindows();
    ...
    console.writestringln('[driver.storage.ctl.ide] (load) driver.storage.ctl.ide Device Loading Finished');

    i := 0;

    if (primaryDevices[0].exists) then begin
        console.writestringln('[driver.storage.ctl.ide] (load) FOUND Primary Master Device 0');
        i := i + 1;
    end;

    if (primaryDevices[1].exists) then begin
        console.writestringln('[driver.storage.ctl.ide] (load) FOUND Primary Slave Device 1');
        i := i + 1;
    end;

    if (secondaryDevices[0].exists) then begin
        console.writestringln('[driver.storage.ctl.ide] (load) FOUND Secondary Master Device 2');
        i := i + 1;
    end;

    if (secondaryDevices[1].exists) then begin
        console.writestringln('[driver.storage.ctl.ide] (load) FOUND Secondary Slave Device 3');
        i := i + 1;
    end;

    // console.writestringln('[driver.storage.ctl.ide] (load) Found ' + i + ' driver.storage.ctl.ide Devices');
...
begin
    push_trace('ide.init');
    console.writestringln('[driver.storage.ctl.ide] (INIT) BEGIN');
    devID.bus:= biPCI;
    devID.id0:= idANY;
    devID.id1:= $00000001;
    devID.id2:= $00000001;
    ...
    devID.id4:= idANY;
    devID.ex:= nil;
    driver.mgr.register_driver('driver.storage.ctl.ide ATA/ATAPI Driver', @devID, @load);
    console.writestringln('[driver.storage.ctl.ide] (INIT) END');
end;
