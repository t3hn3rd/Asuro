...
begin
    debug.tracer.push_trace('driver.video.dummyFDrawPixel.enter');
...
begin
    //debug.tracer.push_trace('driver.video.basicFFlush.enter');
...
begin
    debug.tracer.push_trace('driver.video.basicFDrawLine.enter');
    ...
    if(x1 = x2) then begin
        ...
        DX1:= core.util.abs(DX);
        DY1:= core.util.abs(DY);
...
begin
    debug.tracer.push_trace('driver.video.basicFDrawRect.enter');
...
begin
    debug.tracer.push_trace('driver.video.basicFFillRect.enter');   
    for Y:=y1 to y2 do begin
...
begin
    ...
    //Initialize our 'DriverMap', a core.ds.hashmap of loadable display drivers.
    DriverMap:= core.ds.hashmap.new;
...
begin
    debug.tracer.push_trace('driver.video.register.enter');
    register:= false;
    Value:= core.ds.hashmap.get(DriverMap, DriverIdentifier);
    if (Value = nil) then begin
        core.ds.hashmap.add(DriverMap, DriverIdentifier, void(EnableCallback));
        register:= true;
    end;
    debug.tracer.push_trace('driver.video.register.exit');
end;
...
begin
    debug.tracer.push_trace('driver.video.enable.enter');
    enable:= false;
    Value:= core.ds.hashmap.get(DriverMap, DriverIdentifier);
    if (Value <> nil) then begin
        enable:= FEnableDriver(Value)(@VideoInterface);
    end;
    debug.tracer.push_trace('driver.video.enable.exit');
end;

procedure DrawPixel(X : uint32; Y : uint32; Pixel : TRGB32);
begin
    //debug.tracer.push_trace('driver.video.DrawPixel.enter');
    VideoInterface.DrawRoutines.DrawPixel(VideoInterface.DefaultBuffer, X, Y, Pixel);
    //debug.tracer.push_trace('driver.video.DrawPixel.exit');
end;

procedure Flush();
begin
    //debug.tracer.push_trace('driver.video.Flush.enter');
    VideoInterface.DrawRoutines.Flush(@VideoInterface.FrontBuffer, @VideoInterface.BackBuffer);
    //debug.tracer.push_trace('driver.video.Flush.exit');
end;
...
begin
    ...
    { Point default buffer back to front (in case driver.video.doublebuffer re-enable fails) }
    VideoInterface.DefaultBuffer := @VideoInterface.FrontBuffer;
