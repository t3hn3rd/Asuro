...
{ 
	Driver->Video->Videotypes - Provides core.types relating to the driver.video framework & driver interface.
	
	@author(Kieron Morris <kjm@kieronmorris.me>)
}
unit driver.video.types;
...
type
    //Arbitrary pointer to a raw driver.video buffer in memory
    VideoBuffer = uint32;
    ...
    TVideoBuffer = record
        ...
        Initialized     : Boolean;
        //Location of the driver.video buffer in memory as a QWORD.
    ...
    //Pointer to a driver.video buffer
    ...
    //Struct representing the whole driver.video driver.
    ...
    //Pointer to a driver.video interface struct.
    PVideoInterface = ^TVideoInterface;

    //(Abstract) Enable method for a driver, called from driver.video to enable driver.
    FEnableDriver = function(VideoInterface : PVideoInterface) : boolean;
    //(Abstract) Register driver, called from a driver to register with the driver.video interface.
    FRegisterDriver = function(DriverIdentifier : pchar; EnableCallback : FEnableDriver) : boolean;
    //(Abstract) Init driver, called from _somewhere_ to start the driver and register it with the driver.video interface.
