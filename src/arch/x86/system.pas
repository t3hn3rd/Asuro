...
type
    ...
    { FPC 3.2.2 INITFINAL table — compiler-generated unit init/finalize table.
      Each unit with an initialization or finalization section gets an entry.
      The table is emitted by the compiler under the symbol 'INITFINAL'. }
    ...
    TInitFinalTable = packed record
        TableCount : longint;
        Procs      : array[1..1] of TInitFinalRec; { open array, actual size = TableCount }
    end;
    PInitFinalTable = ^TInitFinalTable;

    { Procedure type for calling initialization procs from the INITFINAL table }
    TProcedure = procedure;

var
  { Compiler-generated INITFINAL table — contains init/finalize procs for
    all units that have initialization or finalization sections }
...
var
    ...
    proc : CodePointer;
begin
    { Walk the compiler-generated INITFINAL table and call each unit's
      initialization procedure (if non-nil).  This runs before kmain, so
      initialization sections must only do trivial work — write to static
      globals, register boot entries, etc.  No I/O or heap allocation. }
    for i := 1 to InitFinalTable.TableCount do begin
        proc := InitFinalTable.Procs[i].InitProc;
        if proc <> nil then
            TProcedure(proc)();
    end;
end;
...
begin
    ...
    { Walk the INITFINAL table generated by compile_initcalls.sh and call
      each unit's initialization section.  FPC with -n does not insert this
      call automatically, so we do it explicitly here — after basic system
      globals are set up but before kmain proceeds.
      Note: we inline the walk rather than calling fpc_initializeunits
      because compilerproc procedures cannot be called from Pascal code. }
    for i := 1 to InitFinalTable.TableCount do begin
        proc := InitFinalTable.Procs[i].InitProc;
        if proc <> nil then
            TProcedure(proc)();
