...
const
    ...
    V86_EXIT_INT    = $FF;
    { Size of the ring-0 stack used for V86 arch.x86.fault.gpf handling }
...
{ ========================================================================= }
{ V86 real-mode memory helpers (access via core.version mapping at 0xC0000000)     }
{ ========================================================================= }
...
{ ========================================================================= }
{ V86 arch.x86.fault.gpf instruction emulator                                              }
{ ========================================================================= }
...
begin
    ...
    case opcode of
        ...
        io.syslog.log('V86', 'Unhandled opcode: $');
        io.syslog.writehexln(opcode);
        io.syslog.log('V86', 'At CS:IP = ');
        io.syslog.writehex(frame^.CS);
        io.syslog.writestring(':');
        io.syslog.writehexln(frame^.EIP);
        V86Success := false;
        V86Active := false;
...
{ ========================================================================= }
{ Naked arch.x86.fault.gpf handler — replaces IDT gate 13                                  }
{ ========================================================================= }
...
asm
    ...
    { -------------------------------------------------------------------- }
    { Non-V86 arch.x86.fault.gpf: set up core.version segments, BSOD and halt                  }
    { -------------------------------------------------------------------- }
    pushad
    mov ax, $10
    mov ds, ax
...
@v86_handle:
    { -------------------------------------------------------------------- }
    { V86 arch.x86.fault.gpf: save all GPRs, call Pascal emulator                        }
    { -------------------------------------------------------------------- }
    pushad
    mov ax, $10
    mov ds, ax
...
@v86_exit:
    { Exit V86: restore the saved protected-mode core.version context.
    ...
    { Restore core.version stack frame and jump back into v86_int }
...
function v86_int(intNo : uint8; var regs : TV86Regs) : boolean;
begin
    push_trace('arch.x86.v86.v86_int');
    ...
    { Save core.version context and enter V86 mode.
      The exit handler will restore ESP/EBP and JMP to @v86_return. }
    asm
        { Save core.version stack state }
...
var
    tss_base : uint32;
begin
    push_trace('arch.x86.v86.init');
    io.syslog.logln('V86', 'INIT BEGIN.');
    ...
    { SS0 at offset 8 = core.version data segment $10 }
    PUInt16(tss_base + 8)^ := $10;
    { Install TSS descriptor in GDT gate 5 (access $89 = present, DPL 0, TSS available) }
    arch.x86.gdt.set_gate(V86_TSS_GDT_GATE, tss_base, V86_TSS_SIZE - 1, $89, $00);
    arch.x86.gdt.reload;
    ...
    { Identity-map the first 4MB at virtual 0x00000000 with User bit.
      This allows V86 code to access the IVT, BDA, driver.video ROM, and
      VGA memory as real-mode linear addresses go through paging. }
    arch.x86.memory.virtual.map_page_user(0, 0);

    { Override IDT gate 13 (#GP) with our naked V86-aware handler }
    arch.x86.idt.set_gate(13, uint32(@v86_gpf_isr), $08, ISR_RING_0);
