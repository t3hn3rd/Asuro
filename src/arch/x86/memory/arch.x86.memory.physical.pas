...
const
    ...
    RESERVED_HI = 3;           { last reserved block (inclusive) — core.version/BIOS }

var
    { Bit set = block physically present (from arch.x86.multiboot memory map) }
...
var
    first, last, i: uint32;
begin
    push_trace('arch.x86.memory.physical.set_memory_area_present');
    first := base shr 22;
...
var
    ...
    i       : uint16;
begin
    push_trace('arch.x86.memory.physical.walk_memory_map');
    ...
    { Walk arch.x86.multiboot memory map entries }
...
procedure force_alloc_block(block: uint16; caller: uint32);
begin
    push_trace('arch.x86.memory.physical.force_alloc_block');
...
procedure init;
begin
    push_trace('arch.x86.memory.physical.init');
    io.syslog.logln('PMM', 'INIT BEGIN.');
    walk_memory_map;
    { Reserve first 16 MiB for core.version / BIOS }
    ...
    io.syslog.log('PMM', ' ');
    io.syslog.writeint(nPresent);
    io.syslog.writestringln('/1024 Blocks Available for Allocation.');
    io.syslog.log('PMM', ' Free: ');
    io.syslog.writeint(pmm_free_blocks);
    io.syslog.writestringln(' blocks.');
    io.syslog.logln('PMM', 'INIT END.');
    pop_trace;
end;

function alloc_block(block: uint16; caller: uint32): boolean;
begin
    push_trace('arch.x86.memory.physical.alloc_block');
    alloc_block := false;
    if block >= BLOCK_COUNT then begin
        GPF;
        pop_trace;
...
var
    ...
    idx    : uint16;
begin
    push_trace('arch.x86.memory.physical.new_block');
    new_block := 0;
    ...
    io.syslog.logln('PMM', 'ERROR: Out of physical memory!');
    pop_trace;
end;

procedure free_block(block: uint16; caller: uint32);
begin
    push_trace('arch.x86.memory.physical.free_block');
    if block >= BLOCK_COUNT then begin
        GPF;
        pop_trace;
        exit;
    ...
    if not bit_test(PhysAlloc, block) then begin
        ...
        io.syslog.logln('PMM', 'WARNING: Double-free on block!');
        GPF;
        pop_trace;
        exit;
    end;
