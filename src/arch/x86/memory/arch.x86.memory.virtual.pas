...
function new_page_directory : uint32;
begin
     push_trace('arch.x86.memory.virtual.new_page_directory');
...
begin
    push_trace('arch.x86.memory.virtual.new_kernel_mapped_page_directory');
...
begin
    push_trace('arch.x86.memory.virtual.load_current_page_directory');
    asm
        MOV EAX, CR3
        MOV Directory, EAX
    end;
...
begin
    push_trace('arch.x86.memory.virtual.init');
    io.syslog.logln('VMM','INIT BEGIN.');
...
begin
    push_trace('arch.x86.memory.virtual.map_page_ex');
    map_page_ex:= false;
...
begin
    push_trace('arch.x86.memory.virtual.map_page');
...
begin
    push_trace('arch.x86.memory.virtual.vtop');
    idx:= address SHR 22;
...
function page_mappable(page_number : uint16) : boolean;
begin
    push_trace('arch.x86.memory.virtual.page_mappable');
    page_mappable := false;
    if not PageDirectory^[page_number].Present then begin
        if not PageDirectory^[page_number].Reserved then begin
            page_mappable := true;
...
begin
    push_trace('arch.x86.memory.virtual.new_page_at_address');
...
begin
    push_trace('arch.x86.memory.virtual.free_page');
    if PageDirectory^[page_number].Present then begin
        ...
        arch.x86.memory.physical.free_block(block, uint32(PageDirectory));
    end else begin
        GPF;
    end;
    pop_trace;
...
begin
    push_trace('arch.x86.memory.virtual.free_page_at_address');
    page_number:= address SHR 22;
...
begin
    push_trace('arch.x86.memory.virtual.map_page_user');
