/**
 * @file lv_conf.h
 * LVGL configuration for Asuro bare-metal kernel
 * Color depth 32 (XRGB8888) to match VESA framebuffer
 */
...
/* 32-bit color to match our VESA XRGB8888 framebuffer */
...
/* Use LVGL's built-in implementations — no libc available */
...
#if LV_USE_DRAW_SW == 1
    /* Only enable the color format we actually use */
...
#define LV_ASSERT_HANDLER           {}  /*no-op: don't hang the kernel on assert*/
...
/* Enable only the fonts we need — Montserrat 14 as default */
...
/* All disabled — we handle display/input ourselves */
#define LV_USE_SDL          0
#define LV_USE_X11          0
